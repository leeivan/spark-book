...
- `平台差异`：说明 Standalone、YARN、Kubernetes、湖仓、对象存储等环境差异。
- `历史迁移`：明确旧 API、旧范式或存量系统代码的阅读方式与迁移边界。

### 禁止扩写的内容

- 不把 Databricks、Dataproc、Delta Lake 的专有能力写成 Apache Spark 通用事实。
- 不把 Spark 2.x、DStream、旧版 `spark.mllib` 资料重新写回现代主线。
...
- 建议补充：
  ...
  - SQL 服务入口与统一计算层的语境
- 禁止扩写：
  - 不把 Databricks SQL 写成 Spark SQL 的默认形态
  - 不回到旧式 Hadoop 组件罗列

#### 第 4 章 `04_structured_data`
...
- 建议补充：
  ...
  - Spark SQL 在平台中的角色
  - Catalyst / Tungsten 的工程意义
- 禁止扩写：
  - 不把平台厂商特性混成 Spark 通用概念
  - 不把 Dataset 写成主线默认入口

#### 第 5 章 `05_stream_processing`
...
- 建议补充：
  ...
  - 流应用与批处理在交付上的相同点与差异
  - Structured Streaming 在 Spark 4.x 中的默认地位
- 禁止扩写：
  - 不让 DStream 再回到主线位置
  - 不用旧式流处理案例替代结构化流主线

#### 第 7 章 `07_machine_learning`
...
- 建议补充：
  - `spark.ml`、Pipeline、评估与调参的工程流程
  - 推荐、分类、聚类案例的训练与验证逻辑
  - 冷启动、特征准备、预测结果解释
- 禁止扩写：
  - 不把 `spark.mllib` 历史 API 写回现代主线
  - 不把深度学习框架内容扩写成 Spark 原生能力

#### 第 8 章 `08_feature_engineering`
...
#### 第 10 章 `10_running_applications`
...
#### 第 2 章 `02_spark_execution_model`

- 优先资料：`Learning Spark`
- 辅助资料：`Hands-on Guide to Apache Spark 3`
- 建议补充：
  - 执行模型、谱系、缓存、Stage / Shuffle 的现代解释
- 编辑策略：
  - 只做精修，不大扩写

#### 第 3 章 `03_pair_rdd_and_partitioning`

- 优先资料：`Learning Spark`
- 辅助资料：`High Performance Spark`
- 建议补充：
  - 分区器、键值对操作、Shuffle 成本的工程表达
- 编辑策略：
  - 只补边界与代价，不回到 RDD 为主线的写法

#### 第 6 章 `06_graph_processing`

- 当前判断：
  - 本地资料库对图计算帮助有限
- 编辑策略：
  - 继续以 GraphX 专题能力定位为主，不额外扩写
...
3. 先决定这次只补哪一类内容：`背景补强 / 概念解释 / 案例工程化 / 平台差异 / 历史迁移`。
...
- 重点动作：
  - 用现代数据平台视角收紧导语和过渡段
  - 补 Spark SQL / DataFrame 在平台中的工程角色
...
- 目标章节：`10`、`11`
- 重点动作：
  - 用 `High Performance Spark` 补交付与调优的工程路径
...
- 目标章节：`07`、`08`、`09`
- 重点动作：
  - 补 Pipeline、评估、算法选型与特征链路
  - 让案例更像项目流程，而不是 REPL 记录
...
- 目标章节：`05`
- 重点动作：
  - 补事件时间、水位线、状态与输出模式的项目语境
  - 保持 DStream 只作为历史兼容内容
