# 5.5 离散化流（DStream，历�?API�?

> 兼容性说明：本节介绍 DStream / Spark Streaming，用于理�?Spark 微批模型与维护历史系统。在 Spark 4.x 的新项目中，不应再以 DStream 作为首选实现模板；请将 5.8 �?5.9 视为本章主线�?

Spark Streaming �?Spark 早期的流处理扩展，其核心抽象�?DStream。它通过把连续数据流切成一系列小批次，再把每个批次表示为底�?RDD 来执行计算。这个模型帮�?Spark 以较低改造成本获得了流处理能力，也解释了为什么很多老系统仍保留 `StreamingContext`、`socketTextStream`、`foreachRDD` �?API�?

从今天的工程视角看，DStream 更适合回答两个问题：一�?Spark 的微批模型是怎样形成的；二是旧系统代码为何大量围�?RDD 和批次间隔编写。理解这些之后，再回头看 Structured Streaming，会更容易理解“无界表 + 增量执行”的统一模型�?

Spark Streaming总体架构通常由以下组件构成（图例 5�?）。首先是要处理的数据必须来自某个外部动态数据源，例如传感器、移动应用程序、Web客户端、服务器日志等等，这个数据通过消息机制传送给数据采集系统，如Kafka、Flume等，递送或沉积在文件系统中�?
...
Spark Streaming的内部工作原理如下（图例 5�?）。Spark数据流接收实时输入数据流并将数据分成批，然后由Spark引擎进行处理，以批量生成最终的结果流�?

![火流](../media/05_stream_processing/media/image6.png)

图例 5�? Spark Streaming工作原理
...
这段代码展示的是最经典�?DStream �?`NetworkWordCount`。整体链路其实很简单：先用 `SparkConf` �?`StreamingContext` 建立一个本地流式应用，`local[2]` 表示至少给接收数据和处理数据各留一个执行线程，批次间隔设为 10 秒；再用 `ssc.socketTextStream()` �?TCP 套接字持续接收文本行，把每一行拆成单词，映射�?`(word, 1)` 形式的键值对，并在每个微批上�?`reduceByKey()` 统计词频。最后通过 `wordCounts.print()` 输出结果。需要特别记住的是：到这一步为止，程序仍然只是在定义计算链路，真正开始处理数据还要等 `ssc.start()` 被调用�?

在虚拟实验环境中，上面的应用已经完成编译和打包。为了复现实验，先启�?Netcat 作为文本输入源：
...
就这样，第一个终端窗口负责发送数据（代码 5�?），第二个终端窗口负责接收处理数据（代码 5�?）�?

### 5.5.2 迁移对照：DStream �?Structured Streaming
...
�?）`StreamingContext + DStream` -> `SparkSession + DataFrame/Dataset`

�?）`reduceByKeyAndWindow` 等窗口聚�?-> `groupBy(window(...))`

�?）`ssc.checkpoint(...)` -> `writeStream.option("checkpointLocation", ...)`
...
�?SparkConf 创建，其指定了Master URL和应用程序名称等其他配置信息�?
...
�?）如果是在Spark应用程序中，StreamingContext对象可以从SparkConf对象创建�?
```scala
...
`appName` 是应用在监控界面里显示的名称。`master` 可以�?Standalone、Kubernetes、YARN 的集群地址，也可以是本地模式字符串 `local[*]`。实际部署到集群时，通常不会�?`master` 硬编码在程序里，而是通过 `spark-submit` 在提交阶段传入；本地测试或单元测试时，`local[*]` 则更方便。创�?`StreamingContext` 时会连带创建底层 `SparkContext`，可通过 `ssc.sparkContext` 访问。批次间隔本身则需要结合延迟目标和集群资源来权衡设置�?

�?）如果通过spark-shell打开交互界面，StreamingContext对象也可以从现有的SparkContext对象创建�?
...
�?）使用StreamingContext.start开始接收数据�?

�?）使用StreamingContext.awaitTermination等待处理停止（手动或由于任何错误）�?

�?）可以使用StreamingContext.stop手动停止处理�?
...
可以使用StreamingContext创建多种类型的输入流，例如receiverStream和fileStream。在代码 5�?中，lines是一个输入离散流，通过socketTextStream从NetCat服务器接收的数据流。每个输入流与接收器（Receiver）对象相关联，该对象接收数据并将其存储在内存中进行处理�?
...
下面我们介绍几种接收器。SocketTextStream已经在代�?5�?中使用了，通过TCP套接字接口接收文本数据，创建一个离散流�?
...
此时，应该看到终端界面中�?0秒刷新一次。现在打开另一个终端界面，将文本文件添加到/data/input目录中：
```bash
