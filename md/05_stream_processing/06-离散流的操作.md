...
我们可以参考代码 5‑2 和代码 5‑3 的执行过程来验证上面的 `transform()` 方法的运行结果。使用 `docker exec` 命令进入容器后，在另一终端运行 Spark 应用程序并观察结果：
```bash
...
这两个参数都必须是源离散流批次间隔的整数倍，图例 5‑9 中源离散流的批次间隔为 1。下面继续扩展前面的示例（代码 5‑1），演示如何在持续 30 秒的数据上每隔 10 秒生成一次单词统计；实现时可以使用 `reduceByKeyAndWindow` 操作。
```scala
...
我们可以参考代码 5‑2 和代码 5‑3 的执行过程来验证上面的 `updateStateByKey()` 方法的运行结果。使用 `docker exec` 命令进入容器后，在另一终端运行 Spark 应用程序并观察结果：
```bash
