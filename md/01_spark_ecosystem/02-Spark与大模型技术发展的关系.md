&emsp;&emsp;﻿﻿﻿# 1.2 Spark与大模型技术发展的关系

&emsp;&emsp;大模型技术从“参数规模竞争”进入“工程化落地竞争”阶段后，数据质量、训练样本构建、知识更新速度和成本控制成为关键。Spark在这一阶段的定位不是替代PyTorch等训练框架，而是承担大模型系统中的数据与计算中枢。

1. 训练与微调数据准备：利用Spark SQL/DataFrame完成多源数据清洗、去重、脱敏、切分、质量校验，并把结果统一落地为Parquet/Delta等格式，为预训练、SFT和偏好优化提供高质量样本。
2. 检索增强（RAG）数据管道：使用Spark批量处理文档、构建分块数据集、生成向量并写入向量库或湖仓，支持知识库的周期性增量更新。
3. 离线评测与效果分析：对模型输出进行批量评测（准确率、召回率、幻觉率、延迟与成本），并结合实验分组做A/B分析，帮助团队快速迭代提示词与模型版本。
4. 生产级批流一体处理：结合Structured Streaming可持续处理日志、会话、反馈与标注回流数据，形成“数据采集-训练/微调-评测-再训练”的闭环。
5. 企业级治理能力：通过统一元数据、权限和审计机制，把大模型数据链路纳入现有数据治理体系，满足合规与可追溯要求。

&emsp;&emsp;在工程实践中可以把Spark理解为“大模型应用的数据底座与管道平台”：模型本身负责学习与推理，Spark负责让数据持续可用、可管、可迭代。

&emsp;&emsp;从现代数据平台视角看，Spark生态不应再被理解为“把各种大数据组件简单堆在一起”，而应理解为以Spark Core和Spark SQL为执行内核、以湖仓和对象存储为主要数据底座、以YARN或Kubernetes为主要资源调度入口的数据处理体系。本章会保留少量Hadoop生态背景，帮助理解Spark的历史位置，但会明确区分当前主线与历史组件。

&emsp;&emsp;在Spark 4.x的工程实践中，批处理、交互式分析和流处理通常围绕DataFrame/Dataset、Spark SQL与Structured Streaming展开；机器学习主线以`spark.ml`管道式API为主，图计算则更多作为专题能力出现。Spark既可以从HDFS、Amazon S3、HBase等持久层读取数据，也可以通过Shell、Spark Submit、Notebook、SQL服务等多种入口承载不同类型的应用。

&emsp;&emsp;Spark支持多种编程语言，包括Java、Scala、Python以及R。对于Spark每个功能的讲解，大部分使用Scala语言，并包括语法讲解，有些使用Java和Python语言用于对比和参考。本书使用Docker容器技术部署Spark实验学习环境，其中包括Spark 4.1.1、HBase 1.4.0，以及Maven和sbt等工具。
