...
如果只看名字，`TF-IDF`、`Word2Vec`、`CountVectorizer` 很容易像是三种彼此独立的工具；但在工程上，它们回答的是不同层面的问题。`HashingTF` �?`CountVectorizer` 更适合保留“词出现了多少次”这类词袋信息，常见于文本分类和检索基线；`Word2Vec` 更强调词在上下文中的语义接近性，更适合相似度、召回或语义泛化要求更强的任务。先分清自己更需要“可解释的词频表示”还是“更连续的语义表示”，再选具体组件，通常会更稳�?
...
如果�?$t$ 表示一个词条，�?$d$ 表示一个文档，�?$D$ 表示整个语料库，那么 $TF(t,d)$ 表示词条 $t$ 在文�?$d$ 中的出现次数，�?$DF(t,D)$ 表示词条 $t$ 在语料库 $D$ 中出现过的文档数。IDF用于降低高频通用词的权重�?

$$ IDF(t,D) = \log\frac{|D| + 1}{DF(t,D) + 1} $$

公式 8�?

其中 $|D|$ 是语料库中文档总数。实际实现里通常会在分子分母上做平滑处理，以避免除零并让数值更稳定。最终，TF-IDF 的值就�?TF �?IDF 的乘积：
...
�?Spark 中，构�?TF-IDF 特征时常见有两条路：`HashingTF + IDF`，或�?`CountVectorizer + IDF`。`HashingTF` 不需要先拟合词表，适合大语料和快速原型；代价是可能发生哈希碰撞。`CountVectorizer` 需要先拟合词汇表，但结果更可解释，也更适合需要稳定词典的场景。对很多入门和工程样例来说，先用 `Tokenizer -> HashingTF -> IDF` 就足够说明主流程�?

下面这段代码演示的是最常见的一条文本特征提取链路：先用 `Tokenizer` 把句子切成词序列，再�?`HashingTF` 把词袋映射成固定维度的稀疏向量，最后用 `IDF` 压低高频通用词的影响。经过这三步后，原始文本就变成了可以直接喂给分类、聚类或检索模型的数值特征�?
...
补充说明：`HashingTF` 背后的关键思想，是用稳定的哈希函数把词条直接映射到固定长度的特征空间里，而不必先维护一份显式词表。Spark 默认使用的就�?MurmurHash3。对本章来说，真正需要记住的不是它的历史背景，而是这类方法的工程权衡：速度快、内存占用稳定，但会有哈希碰撞，因此可解释性不如显式词表方案�?
...
其中 $k$ 是训练窗口大小。可以把 Skip-Gram 理解成“给定中心词，预测窗口内上下文词”的任务。形式化地说，每个词会关联到若干向量参数；当给定�?$w_{j}$ 时，模型会估计上下文�?$w_{i}$ 出现的概率，Softmax 形式可以写成�?
...
�?Spark 里，`Word2Vec` 是一个估算器：它读取分词后的序列，训练出 `Word2VecModel`，再把每个词映射到固定维度的向量。默认做法是把文档中各词向量做聚合，得到文档级表示。这样一来，后面的分类器、聚类器或相似度计算模块就可以直接消费这些向量特征�?
...
补充说明：Softmax 函数常见于多分类模型。可以把它理解成“把一组线性打分转换成一组可比较的概率”，因此它不只出现在 Word2Vec，也常见于多项逻辑回归、神经网络等模型。给定样本向�?$\mathbf{x}$ 和第 $\mathbf{j}$ 个类别对应的参数向量 $\mathbf{w}$ ，其预测概率可以写成�?

$$ P(y = j \mid \mathbf{x}) = \frac{e^{\mathbf{x}^{\mathsf{T}}\mathbf{w}_{j}}}{\sum_{k = 1}^{K}e^{\mathbf{x}^{\mathsf{T}}\mathbf{w}_{k}}} $$

公式 8�?

这可以看作是 $K$ 个线性函�?$x \mapsto \mathbf{x}^{\mathsf{T}}\mathbf{w}_{1}, \ldots, x \mapsto \mathbf{x}^{\mathsf{T}}\mathbf{w}_{K}$ �?softmax 函数的组合（其中 $\mathbf{x}^{\mathsf{T}}\mathbf{w}$ 表示向量 $\mathbf{x}$ �?$\mathbf{w}$ 的内积）。该操作相当于把�?$\mathbf{w}$ 定义的线性变换应用到向量 $\mathbf{x}$ 上，从而将原始的、可能是高维的输入向量映射到 $\mathbf{R}^{K}$ 空间中�?

直观理解：Word2Vec 常见的两种训练思路�?Skip-Gram �?CBOW。可以把它们简单理解成两个方向相反的任务：Skip-Gram 用中心词预测上下文，CBOW 用上下文预测中心词。这里保留这张图，主要是帮助读者建立直观印象；�?Spark 的日常使用中，更重要的是理解输入列、输出向量以及它们在 Pipeline 中处于什么位置�?

![](../media/08_feature_engineering/media/image1.tiff)

图例 8�? Skip-Gram �?CBOW 两种模型

### 8.2.3 CountVectorizer

`CountVectorizer` �?`CountVectorizerModel` 用来把文档集合转换成词频向量。和 `HashingTF` 相比，它会先根据训练语料构建显式词汇表，因此输出结果更可解释，也更适合需要稳定特征字典的场景�?
...
这里每一行都是一�?`Array[String]` 形式的分词结果。`CountVectorizer` 在这组数据上拟合后，会保�?`(a, b, c)` 这样的高频词表，并把每条文本转换成同一特征空间里的词频向量�?
