...
在拟合过程中，`CountVectorizer` 会按词频挑选词汇，并用 `minDF` 之类的参数过滤过于稀疏的词。它还支持把“出现次数”改成“是否出现”的二值表示。下面这个例子展示的是最基本的用法：从分词后的列里学出一个小词表，再把每条文本映射成稀疏向量。
...
这里每一行都是一个 `Array[String]` 形式的分词结果。`CountVectorizer` 在这组数据上拟合后，会学出一个包含 `(a, b, c)` 的小词表，并把每条文本转换成对应的词频向量：
