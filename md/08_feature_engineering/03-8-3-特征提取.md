...
`HashingTF` 背后的关键思想，是用稳定的哈希函数把词条直接映射到固定长度的特征空间里，而不必先维护一份显式词表。Spark 默认使用的就是 MurmurHash3。对本章来说，真正需要记住的不是它的历史背景，而是这类方法的工程权衡：速度快、内存占用稳定，但会有哈希碰撞，因此可解释性不如显式词表方案。
...
Word2Vec 常见的两种训练思路是 Skip-Gram 和 CBOW。可以把它们简单理解成两个方向相反的任务：Skip-Gram 用中心词预测上下文，CBOW 用上下文预测中心词。这里保留这张图，主要是帮助读者建立直观印象；在 Spark 的日常使用中，更重要的是理解输入列、输出向量以及它们在 Pipeline 中处于什么位置。
