...
满足这组条件的哈希族称为 $(r_{1},\ r_{2},\ p_{1},\ p_{2})$-sensitive。在 Spark 中，局部敏感哈希（LSH）相关能力以 `spark.ml.feature` 下的独立类提供，例如 `BucketedRandomProjectionLSH` 和 `MinHashLSH`。它们主要服务于两类问题：近似相似连接，以及近似最近邻搜索。
