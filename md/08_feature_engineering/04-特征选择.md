...
如果把 `VectorSlicer` 看作“按已知业务判断手工裁剪”，把 `RFormula` 看作“快速生成可建模特征列”，那么从这里开始，特征选择就正式进入“让统计信号帮我们筛掉一批候选维度”的阶段了。

### 8.4.4 UnivariateFeatureSelector

如果你希望用更统一的接口处理不同类型的标签与特征，`UnivariateFeatureSelector` 会比 `ChiSqSelector` 更通用。它把“单变量统计检验 + 阈值筛选”这条思路做成了统一入口：可以根据任务类型和特征类型，自动选择更合适的检验方式，再按固定数量、比例或统计阈值筛选特征。对工程实践来说，它的价值在于减少“明明还是做单变量过滤，却要为不同任务换一套 API”的切换成本。

这类方法依然属于过滤式特征选择。也就是说，它们通常发生在模型训练之前，重点是先用统计信号快速排除一批低价值维度，而不是像嵌入式方法那样把特征选择和模型拟合完全绑在一起。因此，当你手里已经有一列统一的 `features` 向量，又希望先做一轮轻量筛选时，`UnivariateFeatureSelector` 往往是比手工堆条件更整洁的选择。
...
它的适用边界也很明确：方差低并不等于一定无用，方差高也不等于一定有预测力。因此，`VarianceThresholdSelector` 更适合放在特征工程前段，先清理掉明显缺乏变化的维度；后面如果还需要进一步收敛，再配合 `ChiSqSelector`、`UnivariateFeatureSelector` 或模型驱动的筛选方法继续细化。
