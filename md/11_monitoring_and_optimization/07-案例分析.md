...
如果应用是 Structured Streaming，这套 UI 视角还会多出一个 Structured Streaming 页面。它会展示每条流查询的输入速率、处理速率、最近批次耗时、状态算子指标，以及 source / sink 的运行状态。对流任务来说，这一页和 Jobs / Stages 是互补关系：前者更像“系统是不是跟得上实时输入”，后者才负责继续下钻到具体哪个阶段或哪个任务在变慢。
...
如果这里分析的是 DataFrame / SQL 作业，还要额外留意 AQE 是否已经在运行时改写了原始计划。现实里很常见的一种误判，是只看提交前的逻辑计划或静态 `EXPLAIN`，就认定某个 Join 一定会按某种方式执行；但启用 AQE 后，真正决定性能的往往是运行时统计和最终执行计划。因此，观察 Join 类型、分区合并和倾斜处理时，应优先结合 Web UI 和最终执行结果来判断，再决定是否需要额外 hint。
