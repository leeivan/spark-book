...
�?）作业（Job）：一次动作操作触发的一次完整执行过程，例如 `count`、`collect` 或写出结果�?

�?）阶段（Stage）：作业会被拆分成若干阶段，阶段通常以Shuffle边界为分隔点；同一阶段内的计算可以流水线执行�?

�?）任务（Task）：阶段内部的最小执行单元，通常一个分区对应一个任务�?

�?）有向无环图（DAG）：由RDD及其依赖关系构成的逻辑图，用来描述“先做什么、后做什么”�?

�?）执行器（Executor）：负责实际执行任务、缓存数据并回传状态的进程�?

�?）驱动程序（Driver Program）：负责编排作业、生成执行计划、向执行器分发任务的进程�?

�?）管理节�?/ 工作节点：旧资料里常�?`Master / Slave` 说法，本书统一改为更中性的“管理节�?/ 工作节点”。实际部署时，Driver并不一定固定运行在某个管理节点上，而是取决于部署模式与集群管理器�?

### 11.2.1 依赖关系

Spark中的所有作业都由一系列操作组成，例�?`map`、`filter`、`reduce` 等，这些操作及其依赖关系共同构成DAG。更准确地说，Spark会先记录逻辑依赖，再在生成物理执行计划时尽量把可流水线化的窄依赖操作合并到同一阶段中执行。像 `map` 后接 `filter` 这样的链式转换，重点不在于“随意改写顺序”，而在于Spark会基于依赖关系与执行边界减少不必要的阶段切分与数据落盘�?

RDD 的一个关键特征是延迟求值。也就是说，当我们连续调�?`map`、`filter`、`reduceByKey` 这类转换时，Spark 先记录“应该怎样计算”，而不是立刻把结果全部算出来。每个新 RDD 都会保留指向�?RDD 的依赖信息，这些依赖关系串起来后，就形成�?DAG，也就是常说的谱系图（lineage graph）。下面通过一个简单例子来观察这张图是如何构成的�?
...
从这个输出可以看到，Spark 并不会只记住“当前这�?RDD 是什么”，还会记住它是由哪些父 RDD、通过哪些转换一步步得到的。正是这些依赖元数据，使 Spark 能在真正执行时生成作业与阶段，也能在分区丢失时按谱系重新计算�?

从实现视角看，Spark 围绕“分区数�?+ 转换依赖 + 失败重算”这三件事组织执行模型。RDD 提供并行数据抽象，DAG 记录转换之间的关系，而调度器则依据这些关系来切分阶段、分发任务并处理失败恢复�?

![http://datastrophic.io/content/images/2016/03/Spark-Overview--1-.png](../media/11_monitoring_and_optimization/media/image2.png)

图例 11�? Spark 应用程序的执行过�?

在高层视角下，一�?Spark 应用通常�?Driver 进程、SparkContext �?SparkSession，以及用户代码共同组成。用户代码声明输入、转换和动作；Spark 再把这些声明转成 DAG，并交给调度器去切分阶段、生成任务、提交到执行器上运行�?
...
几乎所�?Spark 工作流都可以抽象成同一条链路：读入数据，经过若干转换，再通过动作得到结果。决�?Stage 如何切分的关键，不是转换名字本身，而是这些转换形成的是窄依赖还是宽依赖�?
...
<!-- end list -->
  ...
  - > 父RDD的每个分区被子RDD的最多一个分区使�?

  - > 允许在一个集群节点上进行流水线的执行

  - > 故障恢复更有效，因为只有丢失的父分区需要重新计�?
...
<!-- end list -->
  ...
  - > 父RDD的每个分区被子RDD的多个分区使�?

  - > 要求所有父分区的数据可用，并在节点间进行洗�?

  - > 如果某个分区丢失了所有的祖先，则需要完整的重新计算

对于窄依赖，�?RDD 的每个分区最多只会被下游一个分区使用，因此数据通常可以在本地顺序处理，不需要跨节点重分布，例如 `map`、`flatMap`、`filter` �?`sample`。而宽依赖意味着多个下游分区都要读取同一个上游分区的结果，这时就往往需要跨分区重组数据，也就是 Shuffle；`reduceByKey`、`groupByKey`、`join` 等操作都容易形成这样的边界。调度器正是基于这些边界来切�?Stage：能流水线化的窄依赖会尽量合并在同一阶段里，一旦出�?Shuffle 或需要从已物化结果重新开始，就会形成新的阶段�?
...
图例 11�? 通过打破Shuffle边界处的DAG创建Stage
...
第一个RDD `file` 是通过 `sc.textFile()` 创建�?`HadoopRDD`；这条谱系上的最后一个RDD `wordcount` 则是�?`reduceByKey()` 产生�?`ShuffledRDD`。如图例 11�? 所示，左侧展示的是 `wordcount` 对应的逻辑DAG，图中小方块表示各RDD的分区；当调�?`collect` 这类动作时，Spark调度器会在此基础上生成对应的物理执行计划（Physical Execution Plan）并把它拆分为可提交的阶段和任务�?

![](../media/11_monitoring_and_optimization/media/image7.jpeg)

图例 11�? 创建一个物理执行计�?

调度器会根据依赖关系把DAG拆成多个阶段。由于窄依赖通常不需要跨节点重分布数据，因此往往会被合并到同一个阶段里执行；而一旦出�?`reduceByKey` 这类引入Shuffle的操作，就会在此处形成新的阶段。这个例子最终得到两个阶段：前面�?`textFile`、`flatMap`、`map` 属于第一个阶段，生成 `ShuffledRDD` 及其后续动作则进入第二个阶段�?
...
图例 11�? Hadoop 集群中的 Spark 应用程序

然后，可以使用Spark
Web界面来查看Spark应用程序的行为和性能，网址�?http://localhost:4040>，这是运行单词计数作业后的Web
UI的屏幕截图（图例 11�?0）。在Jobs选项卡下，将看到已安排或运行的作业列表，在此示例中是计数的collect作业，Jobs页面显示作业、阶段和任务进度�?

![](../media/11_monitoring_and_optimization/media/image10.jpeg)

图例 11�?0 Jobs页面显示作业、阶段和任务进度
