...
这个案例的目标是：用 DataFrame 定义结构化数据，使用 Spark 提供的结构化 API 完成探索分析，并顺带理解临时视图、SQL 查询和 UDF 的基本配合方式。
...
在团队协作里，这类案例还有一个很常见的演化路径：分析初期，大家往往先通过临时视图和几条 SQL 快速确认业务口径；一旦查询逻辑稳定下来，就再把它整理成可复用的 DataFrame 链式代码，放进正式应用、Notebook 模板或测试用例里。也正因为如此，本节会同时保留 DataFrame API 和 SQL 两种入口，让读者看到它们如何围绕同一份结构化数据协同工作。
...
从现有 RDD 创建 DataFrame，常见有两条路径：一种是已知对象结构时，通过案例类和反射推断 Schema；另一种是显式构造 `StructType` 与 `Row`，在运行时把 Schema 应用到 RDD 上。前者更简洁，后者更灵活。下面先看第一种方式，它可以拆成 4 个动作：读入原始 RDD、定义案例类、把每一行映射成案例类对象、再隐式转换成 DataFrame 并注册视图。

下面先看具体实例：

  - 创建RDD

这里先把 CSV 文本读成一个最原始的 `RDD[Array[String]]`。这一步还没有 Schema，主要是为了给后面的案例类映射做准备。
...
接着把每一行数组映射成 `Incidents` 对象。做完这一步后，数据已经从“位置式字符串数组”变成了“带字段语义的对象 RDD”。
...
然后通过 `toDF()` 把 `sfpdCase` 隐式转换为 DataFrame。到了这一步，后续就可以使用过滤、聚合、排序、SQL 查询等结构化操作。
...
把 DataFrame 注册为临时视图后，就可以同时对比 DataFrame API 和 SQL 两种写法。这也是本节案例的重点之一：它们只是表达方式不同，最终都落在同一套结构化执行引擎上。
...
代码 4‑40
这里的关键不是 API 记忆，而是理解 `RowRDD + StructType + createDataFrame()` 这条组合关系：一旦把 Schema 显式补齐，Spark 就能重新回到结构化处理主线。创建出 DataFrame 后，同样可以注册临时视图并继续用 SQL 查询：
...
接下来进入真正的分析阶段。这里故意保留 DataFrame API 和 SQL 两种写法，是为了让读者看到：在结构化处理里，很多问题都可以先写成链式操作，再按需要切换成 SQL 视图查询。我们先从几个很典型的问题开始：
...
代码 4‑47
这个例子说明了结构化处理的一条典型路径：先组织 DataFrame，再通过分组、聚合和排序回答业务问题。如果要把结果继续交给下游系统，还可以直接使用写操作把结果保存到文件或表中，例如：
...
代码 4‑48
这里把 `top5Add` 以 JSON 格式写出，并使用 `overwrite` 模式覆盖旧结果。在真实项目里，更常见的输出目标往往是 Parquet、湖仓表或下游数仓表，而不是只写一个 JSON 文件；不过这个例子足以说明结构化结果可以自然地衔接到写出阶段。

### 4.4.3 按年份组合

数据集中的日期是 `dd/mm/yy` 形式的字符串。为了按年份聚合，首先要把年份从字符串中提取出来。这里保留一个最小 UDF 示例，主要是为了说明 Spark SQL 里“如何定义并复用自定义逻辑”；但从工程实践看，如果内置字符串函数已经足够，通常应优先使用内置函数而不是 UDF。

Spark SQL 和其他查询引擎一样支持用户定义函数，而且既可以在 DataFrame API 中调用，也可以注册后在 SQL 里复用。下面先定义一个最小的年份提取函数。
...
代码 4‑50
上面的例子说明了 DataFrame API 中直接调用 UDF 的方式。如果希望在 SQL 语句里复用同样的逻辑，还可以先把它注册成命名函数，再在 `SELECT` 里引用。
