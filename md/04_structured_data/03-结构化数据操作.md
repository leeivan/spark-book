...
现在我们已经知道如何创建 DataFrame �?Dataset，接下来就进入真正高频的部分：结构化操作。它们和 RDD 转换一样，同样区分转换与动作、同样具备惰性求值，但表达方式更接近关系代数�?SQL。例如投影、过滤、排序、聚合、窗口和联接，本质上都在告诉 Spark“要得到怎样的结果”，而不是逐步手写每一次底层遍历。对工程实践来说，这种声明式表达尤其重要，因为分析、ETL、特征准备和报表查询常常只是“同一套结构化操作”的不同组合方式，也因此更容易被 Catalyst 统一分析和优化（见表�?4�?）�?

表格 4�? 常用Spark SQL结构化数据操�?
...
<tr class="odd">
<td>select</td>
<td>�?DataFrame 中选择一个或多个列，也可以在选择时直接对列做表达式变换；这是最典型的投影操作�?/td>
</tr>
<tr class="even">
<td>selectExpr</td>
...
<tr class="even">
<td>where</td>
<td>更接�?SQL 写法的过滤入口，根据布尔条件保留满足要求的记录�?/td>
</tr>
<tr class="odd">
<td><p>distinct</p>
...
<tr class="odd">
<td>limit</td>
<td>返回�?<code>n</code> 行结果，常用于调试、预览和抽样查看�?/td>
</tr>
<tr class="even">
<td>union</td>
<td>按相�?Schema 合并两个 DataFrame�?/td>
</tr>
<tr class="odd">
<td>withColumn</td>
...
<tr class="odd">
<td>randomSplit</td>
<td>按权重把数据拆成多个 DataFrame，常用于训练�?/ 验证�?/ 测试集切分�?/td>
</tr>
<tr class="even">
<td>join</td>
...
表格 4�? 中的大多数结构化操作，都需要我们显式指定一个或多个列。这里最容易让初学者困惑的点是：有时列可以直接写成字符串，有时又必须写�?`Column` 对象。一个简单判断是，如果只是“按列名引用已有列”，字符串通常就够了；如果要对列做表达式计算、比较、函数调用或别名转换，就应该显式使用 `Column` 类的实例�?

从高层次上看，`Column` 类主要提供以下几类能力：

�?）数学运算，例如加法，乘法等�?

�?）列值或文字之间的逻辑比较，例如相等、大于、小于等�?

�?）字符串模式匹配�?

有关 `Column` 类中可用的所有功能，请参�?Scala 文档中的 `org.apache.spark.sql.Column`。理解这一点后，就不难把握使用边界：一旦出现“列表达式”，就应当把列写�?`Column` 实例，而不是普通字符串。为了减少混淆，表格 4�? 总结了几种最常见的列引用方式�?
...
`col` �?`column` 函数是同义词，在 Spark �?Scala �?Python 函数库中都可用。如果需要经常在 Scala �?Python 之间切换，使�?`col()` 往往更有可移植性；如果主要使用 Scala，`$"列名"` �?`'列名` 这样的语法糖会更紧凑。DataFrame 类本身也提供�?`col()` 方法，它在执行两个或多个 DataFrame 联接、需要区分同名列时尤其有用�?
...
上面的示例说明了列表达式 `'key > 1` 的特点：一旦不再只是“按列名取列”，而是要做比较、函数调用或运算组合，就应该把列写成 `Column` 对象；如果仍然写成普通字符串，就会触发类型不匹配。在后面的结构化操作示例里，这个边界会反复出现�?

### 4.3.2 选择语句（select、selectExpr�?

`select` �?`selectExpr` 都是在做投影：从 DataFrame 里选出全部列或某个子集，并在需要时顺带完成列变换。两者的主要区别在于表达方式：`select` 更适合已经�?DataFrame API 里写好的列表达式，`selectExpr` 更适合快速写 SQL 风格的表达式、别名和函数调用�?
...
代码 4�?
上面的代码创建了一个简单的 DataFrame，包�?`col1`、`col2`、`col3` 三列。接下来先看几种最基本的“按列选取”写法：
...
代码 4�?
如果 `select` 的参数只是字符串列名，就只能表达“按名字取列”；一旦想在选择时直接套用函数或计算表达式，就需要换�?`Column` 对象，例�?`$"col1"`�?
...
代码 4�?
上面�?`select` 中对字段 `col1` 调用�?`upper()`。这里的关键点不是语法糖本身，而是 `upper()` 需要的输入类型�?`Column`，而不是普通字符串。因此，除了使用 `$"col1"` 这样的写法，也可以改�?`selectExpr()`，把表达式直接写�?SQL 风格字符串：
...
代码 4�?0
这里�?`selectExpr()` 使用了两个表达式：一个对 `col1` 调用 `upper()`，另一个把 `col2` 改名�?`col22`。它�?`select()` 本质上都在做投影，只是前者接�?SQL 风格表达式，后者更偏向 DataFrame API。实际工程里，如果逻辑已经写在 DataFrame 链式调用里，继续�?`select()` 往往更统一；如果只是想快速表达几�?SQL 风格列变换，`selectExpr()` 会更省力�?
...
代码 4�?1
代码
4�?2需要两个列表达式：加和乘，都由Column类中的加函数�?）和乘函数（\*）实现。默认情况下，Spark使用列表达式作为结果列的名称，为了使其更具可读性通常使用as函数将其重命名为更易于理解的列名，我们可能会发现select转换可用于将一个或多个列添加到DataFrame�?
...
drop转换只是从DataFrame中删除指定的列，可以指定要删除的一个或多个列名，但是只会删除存在的列名，而不会删除那些不存在的列名。可以通过select转换仅投影要保留的列，而删除不需要，但是如果DataFrame�?00列，而想删除几列，使用drop转换比select转换更易于使用�?
...
`where` �?`filter` 的作用都很直接：只保留满足条件的行。区别主要在表达习惯上，`where` 更接�?SQL 语感，`filter` 更像函数�?API；两者的执行语义是一致的。写条件时，可以使用字符串表达式，也可以使用 `Column` 条件。`filter()` 最常见的几种形式如下：
...
代码 4�?2
其中，`===` �?Column 类中定义的函数，可以判断两列中每行对应的值是否相等；不等于用 `=!=`。`$"col2"` 是语法糖，返�?Column 对象。下面看一�?where 函数�?
...
DataFrame 是不可变的，因此“给现有结果追加更多行”的常见做法不是原地修改，而是使用 `union` 生成一张新的结果表。它要求参与合并的两�?DataFrame 具有兼容�?Schema，至少列的数量、顺序和类型需要能够对应起来�?

而在真实业务里，只看单张表往往不够。很多分析都需要把多个数据集按键关联起来，例如把交易表与用户表连接、把订单表与商品维表连接、把事件日志与设备属性连接。`join` 的作用，就是基于某个连接条件把两侧数据按行对齐，并把相关列组合到同一�?DataFrame 中。这样一来，原本分散在不同数据集里的信息就能放到同一个查询或分析流程里处理。本节介�?Spark SQL 中常见的连接方式及其适用场景�?

执行连接时，通常需要明确两件事：一是连接条件，也就是用哪些列判断两侧哪些行可以匹配；二是连接类型，也就是匹配成功和匹配失败的行分别要不要保留。表�?4�? 概括�?Spark SQL 中支持的常见连接类型�?

表格 4�? Spark SQL中支持的连接类型

| 类型   | 描述                                            |
| ---- | --------------------------------------------- |
| 内连�? | 当连接表达式的值为true时，从两个数据集中返回行�?                   |
| 左外连接 | 即使连接表达式的值为false，也从左数据集中返回行�?                  |
| 右外连接 | 即使连接表达式的值为false，也从右数据集中返回行�?                  |
| 外连�? | 即使连接表达式的值为false，也从两个数据集中返回行�?                 |
...
代码 4�?5
这里仍然是内连接，但换成�?`joinWith`。它和普�?`join` 的最大区别不在连接条件，而在结果结构：`joinWith` 会把左右两侧对象分别保留�?`_1` �?`_2` 里，而不是把所有列扁平化到同一个命名空间�?
...
代码 4�?6
从结果形态上看，`joinWith` 更像“保留两侧对象结构再做配对”，而普�?`join` 更像“把两侧列拼成一张更宽的表”。如果后续逻辑仍然希望明确区分左右两侧对象，`joinWith` 会更自然�?

#### 4.3.7.3 外连�?

代码
4�?8是左外连接，此连接类型的连接数据集包括内部联接的所有行，以及连接表达式计算为false的左侧数据集的所有行。对于那些不匹配的行，它将右数据集的列填充NULL值�?
...
代码 4�?7
代码
4�?9是右外连接，此连接类型的行为类似于左外连接类型的行为，只是对右数据集采用了相同的处理。换句话说，连接数据集包括内部连接的所有行以及连接表达式计算为false的右数据集的所有行。对于那些不匹配的行，它将为左侧数据集的列填充NULL值�?
...
代码 4�?8
代码 4�?9是全外连接，此连接类型的行为组合了左外连接和右外连接的结果�?
...
代码
4�?0为左反连接，通过这种连接类型，我们可以找出左侧数据集中的哪些行在右侧数据集中没有匹配的行，并且连接后的结果只保留左侧数据集中的列�?
...
代码 4�?1 展示的是左半连接。它与内连接相似，但结果中不会带出右侧数据集的列；可以把它理解为“只保留左侧那些能够匹配成功的行”�?
...
交叉连接不需要连接条件，因此语法上最简单；但它也是最容易失控的一种连接方式，因为结果行数等于左右数据集大小的乘积。只要任一侧数据稍大，结果规模就会迅速膨胀。因此在工程里，交叉连接应当始终显式写成 `crossJoin()`，并且只在确实需要笛卡尔组合时使用�?
...
聚合操作的核心目的，是把明细数据汇总成更适合分析和决策的结果。常见形式包括：对整张表直接聚合、先按一个或多个列分组再聚合，以及在窗口上做滚动或分段统计。Spark 内置了大量聚合函数，因此很多统计需求都可以直接写成结构化表达式，而不必手工维护中间状态�?

�?）将DataFrame视为一个组�?

�?）通过使用一个或多个列将DataFrame分为多个组，并对每个组执行一个或多个聚合�?

�?）将DataFrame划分为多个窗口，并执行移动平均、累积求和或排名。如果窗口是基于时间的，则可以使用滚动窗口或滑动窗口来完成聚合�?

在Spark中，所有聚合都是通过函数完成的。聚合函数可以作用于整个DataFrame，也可以作用于经过groupBy划分后的各个分组。Scala语言聚合函数完整列表位于 `org.apache.spark.sql.functions` 对象中。表�?
4�?列出了一组常用聚合函数及其说明�?
...
按字符串列名分组，然后在分组结果上继续做聚合�?

  - def groupBy(cols:Column \*):RelationalGroupedDataset

�?`Column` 表达式分组，更适合那些需要先做列表达式再分组的场景�?
...
代码 4�?5
代码 4�?5中的 `count` 是对 `groupBy` 的分组结果进行计数。这个结果里 `col1` 的显示顺序并未排序，在代�?
4�?6中，我们使用 `sort` 展示 `col1` 的两种排序方式：
...
代码 4�?7
代码 4�?8使用 `withColumnRenamed` 给结果列重命名：
...
代码 4�?9
`agg()` 通常�?`groupBy()` 一起出现。当一个分组上需要同时计算多个指标，例如计数、最大值、平均值、去重数时，`agg()` 往往是最直接的入口。它既支持传�?`Column` 表达式，也支持用字符串映射描述聚合方式。下面列出几种常见形式：
...
尽管 Spark SQL 为大多数常见用例提供了大量内置函数，但在某些情况下，现成函数仍不足以表达业务逻辑，这时才需要考虑用户定义函数（User-Defined Functions，UDF）。更稳妥的工程顺序通常是：先尽量使用内置函数和列表达式；只有当逻辑确实无法自然表达时，再引�?UDF。因为一旦进�?UDF，优化器能够直接理解和下推的语义就会减少�?

UDF 可以�?Python、Java �?Scala 编写，也可以与外部库集成。不过在性能和执行开销上，不同语言实现并不完全等价�?

从概念上讲，UDF 只是“接收输入、返回输出”的普通函数。但从执行角度看，Scala / Java UDF 可以更直接地运行�?JVM 执行器进程中，�?Python UDF 往往需要额外的 Python 进程和序列化开销。因此，�?Spark 作业里使�?Python UDF 时，尤其要关注性能代价�?

使用 UDF 通常包含三个步骤：先编写并测试普通函数；再通过 Spark �?`udf()` 或注册接口把它包装成可执行函数；最后在 DataFrame 代码�?SQL 查询里调用它。在 SQL 查询中使�?UDF 时，需要先显式注册函数名�?
