...
现在我们已经知道如何创建 DataFrame 和 Dataset，接下来就进入真正高频的部分：结构化操作。它们和 RDD 转换一样，同样区分转换与动作、同样具备惰性求值，但表达方式更接近关系代数和 SQL。例如投影、过滤、排序、聚合、窗口和联接，本质上都在告诉 Spark“要得到怎样的结果”，而不是逐步手写每一次底层遍历。对工程实践来说，这种声明式表达尤其重要，因为分析、ETL、特征准备和报表查询常常只是“同一套结构化操作”的不同组合方式，也因此更容易被 Catalyst 统一分析和优化（见表格 4‑2）。
...
`select` 和 `selectExpr` 都是在做投影：从 DataFrame 里选出全部列或某个子集，并在需要时顺带完成列变换。两者的主要区别在于表达方式：`select` 更适合已经在 DataFrame API 里写好的列表达式，`selectExpr` 更适合快速写 SQL 风格的表达式、别名和函数调用。
...
上面的代码创建了一个简单的 DataFrame，包含 `col1`、`col2`、`col3` 三列。接下来先看几种最基本的“按列选取”写法：
...
如果 `select` 的参数只是字符串列名，就只能表达“按名字取列”；一旦想在选择时直接套用函数或计算表达式，就需要换成 `Column` 对象，例如 `$"col1"`：
...
上面在 `select` 中对字段 `col1` 调用了 `upper()`。这里的关键点不是语法糖本身，而是 `upper()` 需要的输入类型是 `Column`，而不是普通字符串。因此，除了使用 `$"col1"` 这样的写法，也可以改用 `selectExpr()`，把表达式直接写成 SQL 风格字符串：
...
这里的 `selectExpr()` 使用了两个表达式：一个对 `col1` 调用 `upper()`，另一个把 `col2` 改名为 `col22`。它和 `select()` 本质上都在做投影，只是前者接受 SQL 风格表达式，后者更偏向 DataFrame API。实际工程里，如果逻辑已经写在 DataFrame 链式调用里，继续用 `select()` 往往更统一；如果只是想快速表达几个 SQL 风格列变换，`selectExpr()` 会更省力。
...
需要两个列表达式：加和乘，都由Column类中的加函数（+）和乘函数（\*）实现。默认情况下，Spark使用列表达式作为结果列的名称，为了使其更具可读性通常使用as函数将其重命名为更易于理解的列名，我们可能会发现select转换可用于将一个或多个列添加到DataFrame。
...
`where` 和 `filter` 的作用都很直接：只保留满足条件的行。区别主要在表达习惯上，`where` 更接近 SQL 语感，`filter` 更像函数式 API；两者的执行语义是一致的。写条件时，可以使用字符串表达式，也可以使用 `Column` 条件。`filter()` 最常见的几种形式如下：
...
DataFrame 是不可变的，因此“给现有结果追加更多行”的常见做法不是原地修改，而是使用 `union` 生成一张新的结果表。它要求参与合并的两个 DataFrame 具有兼容的 Schema，至少列的数量、顺序和类型需要能够对应起来。
...
这里仍然是内连接，但换成了 `joinWith`。它和普通 `join` 的最大区别不在连接条件，而在结果结构：`joinWith` 会把左右两侧对象分别保留在 `_1` 和 `_2` 里，而不是把所有列扁平化到同一个命名空间。
...
从结果形态上看，`joinWith` 更像“保留两侧对象结构再做配对”，而普通 `join` 更像“把两侧列拼成一张更宽的表”。如果后续逻辑仍然希望明确区分左右两侧对象，`joinWith` 会更自然。

#### 4.3.7.3 外连接

是左外连接，此连接类型的连接数据集包括内部联接的所有行，以及连接表达式计算为false的左侧数据集的所有行。对于那些不匹配的行，它将右数据集的列填充NULL值。
...
是右外连接，此连接类型的行为类似于左外连接类型的行为，只是对右数据集采用了相同的处理。换句话说，连接数据集包括内部连接的所有行以及连接表达式计算为false的右数据集的所有行。对于那些不匹配的行，它将为左侧数据集的列填充NULL值。
...
为左反连接，通过这种连接类型，我们可以找出左侧数据集中的哪些行在右侧数据集中没有匹配的行，并且连接后的结果只保留左侧数据集中的列。
...
#### 4.3.7.5 左半连接
 ...
 展示的是左半连接。它与内连接相似，但结果中不会带出右侧数据集的列；可以把它理解为“只保留左侧那些能够匹配成功的行”。
...
按 `Column` 表达式分组，更适合那些需要先做列表达式再分组的场景。
...
中的 `count` 是对 `groupBy` 的分组结果进行计数。这个结果里 `col1` 的显示顺序并未排序，在中，我们使用 `sort` 展示 `col1` 的两种排序方式：
...
`agg()` 通常和 `groupBy()` 一起出现。当一个分组上需要同时计算多个指标，例如计数、最大值、平均值、去重数时，`agg()` 往往是最直接的入口。它既支持传入 `Column` 表达式，也支持用字符串映射描述聚合方式。下面列出几种常见形式：
