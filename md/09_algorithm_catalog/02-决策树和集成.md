...
�?）原生支�?DataFrame �?ML Pipeline�?

�?）分类与回归分别使用独立估算器，接口更清晰�?

�?）可以利�?DataFrame 元数据区分连续特征和类别特征�?
...
决策树是一种贪婪算法，执行特征空间的递归二进制分区。该树为每个最底部（叶子）的分区预测相同的标签,通过从一组可能的分割中选择最佳分割来贪婪地选择每个分区，以使树节点的信息增益最大化。换句话说，在每个树节点处选择的拆分是从集�?$\underset{s}{\text{argmax}}IG(D,s)$ 中选择�?其中 $IG(D,s)$ 是将分割s应用于数据集D时的信息增益�?

| 不纯�? | 任务 | 公式                                                 | 描述                                                                                        |
| ---- | -- | -------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| 基尼指数 | 分类 | $\sum_{i = 1}^{C}{}f_{i}(1 - f_{i})$             | $f_{i}$ 是标�?$i$ 在一个节点的频率�?$\text{C}$ 是唯一标签的个数�?                                      |
| 信息�? | 分类 | $\sum_{i = 1}^{C}{} - f_{i}\log(f_{i})$          | $f_{i}$ 是标�?$i$ 在一个节点的频率�?$\text{C}$ 是唯一标签的个数�?                                      |
...
假设一个分�?$s$ 将大小为 $N$ 的数据集 $D$ 分为两个大小分别�?$N_{\text{left}}$ �?$N_{\text{right}}$ 的数据集 $D_{\text{left}}$ �?$D_{\text{right}}$ ，则信息增益为：

$$
IG(D,s) = Impurity(D) - \frac{N_{\text{left}}}{N}Impurity(D_{\text{left}}) - \frac{N_{\text{right}}}{N}Impurity(D_{\text{right}})
$$

在单机小数据集上，连续特征的拆分候选往往可以直接取唯一值；但在分布式场景下，这样做代价太高。Spark 更常见的做法，是先通过采样和分位数近似把连续特征离散成若干个桶，再从这些桶边界中挑选拆分候选。这里最值得记住的参数就�?`maxBins`：它控制连续特征最多被切成多少个候选区间。桶数越大，拆分通常越细，但计算和通信成本也越高；另外，桶数不能超过样本数，如果发生这种情况，算法会自动下调�?

对于具有 $M$ 个可能值（类别）的分类特征，可以提�?$2^{M - 1} - 1$ 分割候选。对于二进制�?/1）分类和回归，我们可以通过按平均标签对分类特征值进行排序来减少拆分候选的数量。例如，对于具有一个分类特征为(A,B,C)的二元分类问题，标签1的相应比例为(0.2,0.6,0.4)，则分类特征�?A,C,B)排序，两个拆分的候选对象为(A|C,B)�?A,C|B)，“|”表示分割�?

在多类分类中，将尽可能使用所�?$2^{M - 1} - 1$ 可能的拆分，�?$2^{M - 1} - 1$ 大于参数maxBins时，我们使用（启发式）方法类似于用于二元分类和回归的方法，该 $M$ 分类的特征值由不纯度排序，并将得到 $M - 1$ 分割候选项�?

当满足以下条件之一时，递归树构造将在节点处停止�?

�?）节点深度等于maxDepth训练参数�?

�?）没有分割的候选项会导致信息增益大于minInfoGain�?

�?）没有分割的候选项产生每个都至少具有minInstancesPerNode训练实例的子节点�?
...
`categoricalFeaturesInfo` 用来告诉算法“哪些特征是离散类别、每个特征大约有多少种取值”。它本质上是一个从特征索引到类别数的映射；映射里没出现的特征，都会被当作连续变量处理。例�?`Map(0 -> 2, 4 -> 10)` 表示�?0 个特征是二元类别，第 4 个特征有 10 个可能取值。虽然不显式指定时算法也能运行，但如果把真正的类别特征错当成连续特征，树的拆分质量和训练效率都可能受影响。因此，只要你知道哪些列是离散类别，就应尽量明确告诉模型�?
...
当树很深，或者随机森林中的树数量很多时，训练开销往往不再只来自模型本身，还会来自节点匹配和谱系增长。下面这组参数就是为这种“大�?多树”场景准备的�?

`useNodeIdCache`：开启后，算法会缓存样本当前落在哪些节点的信息，而不是在每次迭代时都把整棵树重新广播给执行器。它在深树或大型随机森林里可能明显减少通信和重复匹配成本；但代价是会额外生成一串缓�?RDD，因此通常要和检查点一起使用，避免谱系过长�?
...
下面这段历史示例�?LIBSVM 数据文件读入样本，将其解析成 RDD 形式�?`LabeledPoint`，再用决策树完成分类任务。这里使用的杂质度量�?Gini，最大树深度设为 5，最后再用测试误差做最基础的效果评估�?
...
下面这段历史示例继续沿用 LIBSVM 数据文件，把样本解析�?RDD 风格�?`LabeledPoint`，再训练一个以方差为杂质度量、最大深度为 5 的回归树。收尾时使用均方误差（MSE）评估拟合效果�?
...
DataFrame API 支持两类主要的树集成算法：随机森林（Random Forests）和梯度提升树（Gradient-Boosted Trees，GBTs）。两者都�?`spark.ml` 决策树为基础模型，只是集成方式不同。与早期 MLlib 写法相比，DataFrame API 最重要的变化在于：

�?）对于DataFrame和ML管道支持

�?）分类与回归的分�?

�?）使用DataFrame的元数据来区分连续和分类特征

�?）随机森林具有更多的功能：特征重要性的估计，以及分类中的每一个类的预测概率，也称为类条件概率�?

#### 9.2.2.1 随机森林

随机森林可以看作“多棵决策树的集成”。它通过对样本和特征做随机采样来训练多棵树，再对结果做投票或平均，因此通常比单棵决策树更稳健，也更不容易过拟合。`spark.ml` 中的随机森林支持二元分类、多分类和回归任务，仍以 DataFrame �?`label` �?`features` 列作为标准输入�?
...
| probabilityCol | Vector | "probability" | 归一化后的类别概�?|
...
从历�?`spark.mllib` 的视角看，树集成主要有两类：`GradientBoostedTrees` �?`RandomForest`。两者都以决策树为基础模型，但训练方式和使用取舍并不相同。读这一节时，真正值得关心的是它们在工程上的差异：

�?）梯度提升树一次训练一棵树，因此与随机森林相比，它们的训练时间更长。随机森林可以并行训练多棵树。另一方面，与随机森林相比，梯度提升树使用较小（较浅）的树通常是合理的，并且训练较小的树所需的时间更少�?

�?）随机森林通常更稳健，也更不容易因为树数增加而立刻过拟合；而梯度提升树对迭代数、学习率和树深更敏感，如果继续堆树，验证集效果反而可能下降�?

�?）随机森林可能更容易调整，因为性能随树的数量单调提高，而如果树的数量太大，梯度提升树的性能可能会开始下降�?
...
下面这段历史示例�?LIBSVM 数据解析�?`LabeledPoint`，再训练随机森林分类器，并用测试误差给出最基础的准确性参考�?
...
```
  ...
  - 训练模型，categoricalFeaturesInfo为空表明所有特征为连续�?
...
梯度提升树通过迭代方式训练一串浅层树，每一轮都去修正前面模型尚未解释好的误差，因此往往比随机森林更“能拟合”，但也更依赖参数控制。像决策树一样，它能够处理非线性关系和类别特征，而且通常不要求特征缩放。历�?`spark.mllib` 接口主要支持二元分类和回归；如果面对多类问题，通常更适合优先考虑决策树、随机森林，或者直接转�?`spark.ml` 主线接口�?
...
下表列出了历�?`spark.mllib` 中梯度提升树支持的损失函数。每种损失都各有适用场景，但不会同时覆盖分类和回归两类任务�?
...
loss：有关损失函数及其对任务的适用性的信息，请参见表格 9�?。根据数据集的不同，不同的损失函数可能会产生明显不同的结果�?
...
如果树的数量不断增加，而验证集效果不再改善，就需要及时停下来，而不是盲目继续堆迭代。历史接口里可以通过 `runWithValidation()` 观察这一过程：它接收训练集和验证集两�?RDD，当验证误差的改善幅度低于阈值时就停止训练。实践中，验证误差往往先下降再回升，因此配�?`evaluateEachIteration` 查看曲线，比只盯训练误差更可靠�?

下面这段历史示例�?LIBSVM 数据出发，构�?`LabeledPoint`，再训练一个使用对数损失的梯度提升树分类模型，并用测试误差做基础评估�?
...
```
  ...
  - 训练GradientBoostedTrees模型，分类任务缺省使用对数损失函�?
...
下面这段历史示例把任务切到回归：先读�?LIBSVM 数据并解析成 `LabeledPoint`，再训练一个以平方误差为损失函数的梯度提升树回归模型，最后通过均方误差（MSE）评估拟合效果�?
...
```
  ...
  - 训练GradientBoostedTrees模型，回归任务缺省使用平方误差损失函�?
