...
在案例分析中，将使用飞行数据更详细地描述创建属性图的每一步，使用航班数据作为输入数据，机场是属性图中的点，路线是边。我们使�?014�?月的航班信息，对于每个航班，我们都有以下信息�?
...
边是机场之间的路线，必须具有始发�?org\_id:Long)和目标点(dest\_id:Long)，并且可以具有属性距�?dist:Int)。边RDD具有的形式为(src\_id,
dest\_id, dist）�?
...
下面的代码使用Pregel计算最便宜的机票，其中使用的公式为�?0 +距离/ 20
