...
把核心图操作�?`GraphOps` 拆开，一个重要原因是 GraphX 未来可以支持不同的底层图表示：每种表示只要实现核心能力，就能复用 `GraphOps` 里这批更高层的辅助操作。下面先�?`Graph` �?`GraphOps` 中最常用的一组函数汇总起来；为了便于阅读，正文里统一�?`Graph` 的成员来介绍。这里对部分函数签名做了简化，例如省略默认参数、类型约束和更高级的变体；如果需要精确接口，仍应回到官方 API 文档核对�?
...
与RDD的map()运算符一样，属性图包含mapVertices()、mapEdges()和mapTriplets()等方法，这些运算符中的每一个产生一个新的图，其点或边属性被用户定义的map()函数修改。注意，在每种情况下图结构都不受影响，这是这些运算符的一个关键特征，它允许生成的图重用原始图形的结构索引，以下代码片段（代码 6�?3和代�?6�?4）在逻辑上是等效的，但是第一个片段不保留结构索引，并且不会从GraphX系统优化中受益：
```scala
...
subgraph()使用两个返回布尔值的条件判定函数作为参数，第一个判定函数为epred，输入参数为EdgeTriplet，并在三元组满足该判定条件时返回true；第二个判定函数为vpred，输入一�?VertexId,
...
注意到用�?L没有相应的信息，但是连接到用�?L（peter）和5L（franklin）�?

�?）移除缺失点以及连接到它们的边，通过移出用户0L断开与用�?L（peter）和5L（franklin）的连接
...
柯里�?Currying)函数是一个带有多个参数，并引入到一个函数链中的函数，每个函数都使用一个参数。柯里化(Currying)函数用多个参数表定义，如下所示：
```scala
...
可以根据需要在柯里�?Currying)函数上定义两个以上的参数。尝试下面一个简单的示例程序用来了解如何使用柯里�?Currying)函数�?
```scala
object Demo {
...
如果两位演员共同出演过同一部电影，就在图中为他们建立一条连接边，并把电影标题作为边属性。下面先把这部分数据加载成名�?`movies` 的边 RDD�?
...
接下来用 `joinVertices()` 把这些信息并�?`movieGraph`。先定义一个小函数，把演员家乡追加到演员名字后面：
...
这里需要注意一点：虽然 `bio` 里包�?“Antonio Banderas�?�?“Paul Walker”，但它们并不属于原始图中的节点集合，所�?`joinVertices()` 不会凭空创建新顶点�?
...
首先介绍VertexRDD
和EdgeRDD的映射操作。mapValues()将一个map()函数作为输入参数，该函数转换VertexRDD中的每个点属性，然后返回一个新的VertexRDD对象，同时保留原始的点索引。mapValues()方法被重载，因此map()函数可以采用VD�?VertexId,
...
使用filter()方法，我们还可以过滤VertexRDD集合。在不更改点索引的情况下，filter()删除不满足用户定义判定条件的点。与mapValues()不同，filter()没有重载，因此判定的类型必须�?VertexId,
VD)=\> Boolean，总结如下�?
...
Edge.Direction.In：每个点仅收集具有传入链接的相邻属�?

Edge.Direction.Out：每个点仅收集具有传出链接的相邻属�?

Edge.Direction.Either：每个顶点都收集其所有邻居的属�?

Edge.Direction.Both：每个顶点都收集同时具有传入和传出链接的相邻属�?
...
要创建新的调料网络，我们需要知道哪些成分共享某些合成物。这可以通过首先收集foodNetwork图中每个合成物节点的成分ID来完成，就是将具有相同合成物的成分ID收集并分组到元组的RDD集合�?compound
...
接下来，我们创建一个函数pairIngredients()，参数的类型�?compound id, Array\[ingredient
