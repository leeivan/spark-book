...
这一节先�?Spark 交互环境里构造一个最小社交网络，再用它演�?GraphX 最常见的建图与探索流程。开始之前，先导�?GraphX �?RDD 相关包：
...
在交互环境中，`SparkContext` 通常已经自动创建好，可以直接用来从本地文件系统或分布式存储加载数据。这里我们使�?`/data` 目录下的两个 CSV 文件 `people.csv` �?`links.csv`，分别表示人物节点和人物之间的连接关系：
...
�?CSV 读进来的只是字符串形式的 RDD；要真正构建图，还需要把它们解析成顶点集合和边集合。在 GraphX 里，最核心的抽象是“属性图”：顶点和边都可以携带用户定义的属性对象。`Graph` 类可以粗略理解成下面这样的结构：
```scala
class Graph[VD, ED] {
...
�?）定义一个案例类Person，具有名称和年龄作为类参数，以后需要对Person进行模式匹配时，案例类非常有用：
...
�?）将两个CSV文件中的每一行，分别解析为Person和Edge类型的新对象，并结果集合定义为RDD \[(VertexId,
...
类参�?`srcId` �?`dstId` 表示边的起点和终�?`VertexId`。在这个社交网络示例里，两个人之间的链接是单向的，边属性则通过 `Connection` 类型�?`attr` 字段描述。这里把 `Connection` 定义�?`String` 的类型别名，主要是为了让代码更贴近业务语义，读起来也更直观�?

�?）使用工厂方法Graph()创建社交网络图，并将其命名为tinySocial�?
...
代码 6�?0有两个问题。首先不是很简洁和富有表现力，其次for循环中的过滤操作效率不高，需要读取两遍图中数据。GraphX库提供了两种查看数据的方式：以图的方式或以表的方式，表中可以分别为边、点或三元组。对于每种方式，GraphX库都提供了一系列丰富的操作，并进行了优化。我们通常可以轻松地使用预定义的图操作或算法来处理图形，例如可以简化前面的代码并使之更高效，如下所示：
