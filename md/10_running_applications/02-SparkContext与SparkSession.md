...
�?Spark 4.x 中，应用程序应优先以 SparkSession 作为统一入口。它把结构化数据、SQL、Structured Streaming 以及底层执行上下文组织在同一个对象里，能让应用代码保持更一致的初始化方式�?

历史上常见的 SQLContext、HiveContext 等写法，现在更适合作为演进背景来理解，而不再适合作为新项目模板�?

  - 注意

SparkSession �?Spark 2.0 之后已经�?SQLContext �?HiveContext 统一为单一入口对象�?

可以使用 `SparkSession.builder` 创建或获�?SparkSession 实例�?
```scala
...
完成后可以调�?`stop()` 方法关闭当前�?SparkSession�?
```text
...
早期 Spark 版本�?`spark-shell` 默认暴露的是 `sc`；在 Spark 2.0 之后，交互环境会直接创建名为 `spark` �?SparkSession，同时仍然保留底层的 `sc` 以便访问 SparkContext。也就是说，今天读者在 shell 里通常先接触到的是统一入口 `spark`，而不是多个并列上下文对象�?
```text
...
SparkSession 内部封装�?SparkContext。为了理解提交、调度与资源协作关系，下面先简单看一�?SparkContext 在运行时中的位置�?

![Fig 7. SparkContext as it relates to Driver and Cluster
Manager](../media/10_running_applications/media/image1.png)

图例 10�? SparkContext �?Driver �?Cluster Manager 的关�?

如图所示（图例 10�?），SparkContext是底层执行上下文；每个JVM通常只有一个SparkContext。Spark驱动程序（Driver
Program）通过它连接集群管理器（YARN，Kubernetes或Standalone）并提交作业。业务层代码建议通过SparkSession访问能力，在需要底层控制时再使用`spark.sparkContext`�?

�?Spark 4.x 中，SparkSession 已经是统一入口�? 既可处理 DataFrame、Dataset �?SQL，也能衔接流处理与底层执行上下文。这样可以减少上下文对象切换带来的复杂度，降低工程代码里初始化和配置分散的风险。下面继续介�?SparkSession 常见的类方法与实例方法�?

  - builder(): Builder

> `builder()` 用来创建一�?Builder，再通过它获取或创建 SparkSession 实例。下面这段代码适用�?Scala 应用程序；在 `spark-shell` 中，SparkSession 通常已经由系统自动创建�?
```scala
...
implicits对象提供对从任何类型的RDD（Encoder所包括的）、case类或元组以及Seq创建Dataset，还提供从Scala的Symbol�?到Column的转换，还提供从Product类型（例如案例类或元组）的RDD或Seq到DataFrame的转换，具有从Int、Long和String的RDD到具有单个列名“\_1”的DataFrame直接转换�?
...
> 创建一个空�?Dataset\[T\]。当你已经确定记录类型，但暂时还没有任何数据时，这个方法很方便�?
