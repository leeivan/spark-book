...
`spark-submit` 是 Spark 应用从“本地工程”进入“运行环境”的标准入口。它的价值不只是帮你执行一条命令，更重要的是把入口类、主程序文件、依赖、资源配置和部署模式统一收敛到同一个提交界面里。无论目标环境是本地、Standalone、YARN 还是 Kubernetes，应用的交付方式都尽量保持一致。

sbt 和 Maven 都有装配插件，可以把应用代码及其所需依赖打包成装配 jar 或 uber-jar。通常会把 Spark 和 Hadoop 标记为 `provided`，因为这些组件往往由运行环境提供，不需要重复放进最终产物。打包完成后，就可以通过 `bin/spark-submit` 传入 jar 并启动应用。这个脚本会负责准备类路径、传递参数，并适配不同的集群管理器与部署模式：
...
（4）--conf：`key=value` 形式的任意 Spark 配置项。对于包含空格的值，应使用引号包裹。

（5）application-jar：主程序 jar 的路径。它应当能够被目标运行环境访问，例如 HDFS 路径、对象存储路径，或所有节点都可见的本地路径。

（6）application-arguments：传递给主类 `main` 方法的业务参数。
...
在实际交付里，经常可以把同一个应用看成三个阶段：开发者先在本地或测试环境用 `client` 模式验证参数、日志和输出；进入联调阶段后，再把同一份 jar 或脚本提交到靠近数据的集群环境，确认依赖分发、资源参数和外部连接是否正常；等任务要长期稳定运行时，再转成 `cluster` 模式交给调度系统。这样看，部署模式的选择并不是“哪一个更正式”，而是“当前更需要调试便利，还是更需要运行稳定性与网络就近性”。

有些参数会和具体集群管理器绑定。例如在 Standalone 的 `cluster` 模式下，可以使用 `--supervise` 在 Driver 异常退出时自动重启。要查看完整参数列表，可以执行 `spark-submit --help`。下面先看几类最常用的提交示例：
...
### 10.5.1 结构化流提交示例（Kafka + Kubernetes）

下面给出一个版本相关的最小示例模板，用来说明结构化流任务在 Kubernetes 上提交时通常会包含哪些关键参数。示例里的镜像、Spark 坐标和 Scala 版本以当前书稿使用的 Spark 4.1.1 / Scala 2.13 组合为例；如果你的运行环境版本不同，应同步调整镜像标签、`--packages` 坐标和应用产物名称。
...
除了命令行参数，`spark-submit` 还会合并属性文件中的默认配置。最常见的就是 Spark 安装目录下的 `conf/spark-defaults.conf`。这意味着某些重复出现的参数可以提前固化在默认文件里，而不必每次都写在提交命令中；例如如果已经在默认文件里设置了 `spark.master`，提交时就不必重复显式传 `--master`。通常，运行配置主要来自三个地方：
...
一般来说，应用代码里 `SparkConf` 的显式设置优先级最高，其次是 `spark-submit` 命令行参数，最后才是默认配置文件。如果某个参数的生效来源不清楚，可以使用 `spark-submit --verbose` 查看更详细的解析过程。

使用 `spark-submit` 时，主程序 jar 和 `--jars` 指定的附加 jar 会被自动传播到运行环境。`--jars` 后面可以传入逗号分隔的多个 URL，这些 jar 会同时加入 Driver 和 Executor 的类路径。Spark 支持多种 URL 方案来分发这些文件：

（1）`file:`：表示提交端本地文件。Spark 会通过驱动侧的文件分发机制把它传给执行器。

（2）`hdfs:`、`http:`、`https:`、`ftp:`：表示由各节点按 URI 自行拉取文件。

（3）`local:`：表示文件已经预先存在于每个工作节点的本地路径上，因此运行时不会再做网络分发。它适合镜像内已内置的 jar，或通过共享存储统一下发的大文件。
...
从运行时视角看，一个 Spark 应用就是一组相互协作的集群进程。Driver 负责持有 SparkSession 或 SparkContext、生成执行计划并调度任务；Cluster Manager 负责分配资源；Executor 则在工作节点上实际执行计算、缓存数据并回传结果。理解这三者的关系，是后面看部署模式、资源配置和故障定位的基础。
...
在 Standalone 模式里，还会看到管理节点（Master）和工作节点（Worker）这两个进程角色；它们负责资源管理与进程托管，但不应和 Driver/Executor 简单一一对应。Driver 与 Executor 各自运行在独立的 JVM 进程中，可以部署在同一组机器上，也可以按网络与资源条件分开部署。
...
执行器是 Spark 应用真正执行计算的地方。启动后，执行器会先向 Driver 注册，然后持续接收任务、执行计算、缓存中间结果，并把状态和结果回传给 Driver。每个 Spark 应用都有自己独立的一组执行器；默认情况下，这些执行器会贯穿应用生命周期存在，也可以在启用动态分配时按负载变化动态增减。执行器的主要职责包括：

（1）接收并执行 Driver 下发的任务。

（2）读取和写入外部数据源，或参与 Shuffle 数据交换。

（3）把中间结果缓存在内存或磁盘中，供后续任务复用。

（4）向 Driver 回报状态、指标和任务执行结果。
...
如果要使用 Spark Standalone，最基本的前提就是每个节点都具备可运行的 Spark 环境，并且网络与 SSH 配置满足节点间通信要求。对于教学环境或轻量集群，这通常是最容易搭起来的一种方案。
...
此外，Standalone 支持对异常退出的 Driver 自动拉起。要启用这一能力，可以在提交时使用 `--supervise`。如果某个应用因为配置或代码问题反复启动失败，通常需要先在管理节点的 Web UI 中确认 Driver ID，再手动停止这次提交：
...
Driver ID 可以在 Standalone 管理节点的 Web UI 中找到，例如 `http://<master-url>:8080`。

  - 资源调度

Standalone 默认提供的是跨应用的简单 FIFO 调度。为了避免单个应用独占整个集群，通常会限制每个应用可用的最大 CPU 核心数。否则，在默认设置下，一个应用可能会拿走集群中的全部可用核心。对于共享环境，更稳妥的做法是在 `SparkConf` 里通过 `spark.cores.max` 设置上限，例如：
...
代码 10.29
此外，还可以在管理节点上设置 `spark.deploy.defaultCores`。如果应用没有显式设置 `spark.cores.max`，就会采用这个默认值，而不是尽可能占满全部核心。共享集群里，平台侧先给出一个统一上限通常更省心。做法是在 `conf/spark-env.sh` 中加入：
...
Standalone 模式自带基于 Web 的监控界面，管理节点和每个工作节点都有各自的 UI 页面，用来查看集群资源、已提交应用和运行状态。管理节点的 Web UI 默认通常监听在 `8080` 端口，也可以通过配置或启动参数调整。除了 Web UI 之外，每个应用的详细日志还会写入工作节点的工作目录，默认位置一般是 `SPARK_HOME/work`，其中最常见的就是 `stdout` 与 `stderr` 两类输出文件。排查问题时，通常先在 Web UI 确认应用和 Executor 的状态，再回到对应节点查看日志，会比直接翻目录更高效。

  - 与Hadoop一起运行

如果 Spark 运行在现有 Hadoop 平台旁边，最常见的集成方式不是“把两套系统混成一个角色”，而是让 Spark 作为独立计算层，通过 `hdfs://`、Hive Metastore 和队列配置访问已有数据与资源。这样做的代价，是远程读取数据时可能不如本地磁盘直接访问；但在统一的大数据平台里，它换来的好处往往是更成熟的存储、权限和运维体系。

#### 10.5.3.2 YARN

Apache Hadoop YARN 是 Hadoop 生态中的统一资源管理器。对已经运行在 HDFS、Hive 等平台之上的团队来说，YARN 的优势在于可以直接复用现有的大数据基础设施、用户体系与运维方式，因此 Spark on YARN 仍然是很多传统数据平台里的常见选择。

从 Spark 应用的角度理解 YARN，抓住三个角色就够了：`ResourceManager` 负责全局资源协调，`NodeManager` 负责节点级容器托管，`ApplicationMaster` 负责当前应用的一次提交实例。Spark 驱动和执行器最终运行在哪些容器里、拿到多少资源，都是围绕这三类角色协商出来的。

这套模型最适合的场景，是团队已经把存储、权限、队列和多租户运维都建立在 Hadoop 平台之上。此时 Spark 不需要重新发明一套资源管理机制，而是把计算任务接到现有平台上即可。相应的代价，是配置项和运行链路会比 Standalone 更复杂，排障时通常也需要同时查看 Spark UI、YARN UI 和容器日志。

在实际提交前，先确保 `HADOOP_CONF_DIR` 或 `YARN_CONF_DIR` 指向正确的客户端配置目录。Spark 会依赖这些配置去连接 HDFS、解析队列信息，并把必要的 Hadoop/YARN 上下文分发到相关容器中。如果应用本身还依赖额外的环境变量或 Java 参数，也要通过 Spark 配置把它们显式带进驱动和执行器环境。

YARN 上同样支持 `client` 和 `cluster` 两种部署模式：`cluster` 模式下，驱动程序随应用一起交给 YARN 托管，提交命令在完成提交后即可退出；`client` 模式下，驱动程序保留在发起提交的那台机器上，更适合调试或需要立即观察本地日志的场景。和 Standalone 或 Kubernetes 不同，YARN 的底层地址通常由 Hadoop 配置解析出来，所以提交时的 `--master` 固定写成 `yarn` 即可。下面先看 `cluster` 模式的最小写法：
...
上面的命令会把应用提交给 YARN，并由 YARN 为这次提交拉起对应的 `ApplicationMaster` 和后续容器。提交端会定期轮询运行状态，并把关键信息输出到控制台；应用结束后，本次提交也就完成了。若要切回 `client` 模式，核心差别只是把驱动保留在本地机器上。下面演示如何在 `client` 模式下启动 `spark-shell`：
...
在 `cluster` 模式下，驱动程序运行在 YARN 容器里，而不是提交命令所在的本地机器上。因此，凡是只存在于客户端本地磁盘上的 jar、配置文件或资源文件，都不能指望通过本地路径被远端驱动自动看到；更稳妥的做法，是在提交命令里用 `--jars`、`--files` 或分布式存储路径把它们显式带进去。
