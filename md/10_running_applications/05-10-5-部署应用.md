...
有些参数会和具体集群管理器绑定。例如在 Standalone 的 `cluster` 模式下，可以使用 `--supervise` 在 Driver 异常退出时自动重启。要查看完整参数列表，可以执行 `spark-submit --help`。下面给出几种常见提交示例：
...
除了命令行参数，`spark-submit` 还会从属性文件里读取默认配置。默认情况下，它会加载 Spark 安装目录中的 `conf/spark-defaults.conf`。这意味着某些重复出现的配置不必每次都写在命令行里；例如如果默认文件里已经设置了 `spark.master`，提交时就可以不再显式传 `--master`。通常，运行配置主要来自三个地方：
