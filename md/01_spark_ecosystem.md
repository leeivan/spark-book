...
Shell的行命令、基于Spark Submit的批处理，基于Structured Streaming的实时处理应用、Spark
SQL的即席查询、MLlib的机器学习、GraphX的图处理等等。

Spark支持多种编程语言，包括Java、Scala、Python以及R。对于Spark每个功能的讲解，大部分使用Scala语言，并包括语法讲解，有些使用Java和Python语言用于对比和参考。本书使用Docker容器技术部署Spark实验学习环境，其中包括了Spark的当前版本为4.1.1，HBase版本为1.4.0，另外还有Maven和sbt等工具。
...
YARN或Kubernetes的集群管理。在分布式存储方面，Spark可以和 Alluxio、HDFS、
...
SQL，用于机器学习的MLlib，用于图形处理的GraphX和用来数据流处理的Structured Streaming（图例 1‑2）。
