...
在建模流程里，这一步最有价值的地方，不是“多算几个统计量”，而是帮助我们尽早回答几个实际问题：特征之间是否高度相关、标签与候选特征是否存在基本关联、某些列是否几乎恒定或量纲异常、后续是否需要标准化、离散化或特征筛选。越早把这些问题看清楚，后面的 Pipeline、调参和模型解释就越不容易走弯路。
...
在 `spark.ml` 中，`Summarizer` 用来对 DataFrame 的向量列做摘要统计。它支持的度量包括每一维特征的最大值、最小值、平均值、方差、非零元素数量以及样本总数。对特征工程来说，这一步很适合作为建模前的快速体检：先看各维度量纲是否一致、是否存在全零列，再决定是否继续做缩放、筛选或异常值处理。下面这组 Scala REPL 示例分别演示带权重列和不带权重列时如何计算均值与方差。
