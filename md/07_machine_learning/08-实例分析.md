...
&emsp;&emsp;本节保留两个案例：一个围绕推荐系统，一个围绕分类问题。需要注意，案例中的部分代码仍使用 `spark.mllib` / RDD 风格 API，目的是帮助读者理解历史写法和存量系统；如果在 Spark 4.x 中新建项目，应优先选择 `spark.ml`、DataFrame 和 Pipeline 的等价实现。

### 7.8.1 预测用户偏好

&emsp;&emsp;> 兼容性说明：下面的 ALS 示例使用 RDD 风格写法，适合说明矩阵分解与旧项目代码结构。Spark 4.x 新项目建议优先参考 `org.apache.spark.ml.recommendation.ALS` 的 DataFrame API。

&emsp;&emsp;如果今天重新实现这一类推荐任务，通常会先把评分数据整理成包含 `user`、`item`、`rating` 三列的 DataFrame，再把特征清洗、训练与评估放进更一致的工程流程中。保留下面的写法，是为了帮助读者识别历史推荐系统代码最常见的组织方式。

&emsp;&emsp;从工程流程看，这个案例其实可以拆成四步：先把用户 - 物品交互整理成可训练的数据集，再拆分训练集与测试集，接着训练模型并用误差指标验证效果，最后才是把模型用于推荐输出或冷启动演示。把这四步分开理解，会比单纯盯着 ALS 公式更接近真实项目里的工作方式。

&emsp;&emsp;下面继续用电影推荐说明协同过滤。假设 Ted 喜欢电影 A、B、C，Carol 喜欢电影 B、C，那么系统就会尝试根据这些交互记录推断 Bob 可能喜欢什么。Spark 里的 ALS（Alternating Least Squares，交替最小二乘）正是围绕这类“用户 - 物品评分矩阵”构建的经典矩阵分解方法。

&emsp;&emsp;ALS 的核心思路，是把原始的稀疏评分矩阵分解成两个更低维的稠密矩阵：一个表示用户在若干潜在因子上的偏好强度，另一个表示项目在同一组潜在因子上的表现。用户和项目在这些潜在因子上的匹配程度，决定了模型给出的预测评分。图例 7‑10 展示了这种“把用户兴趣和项目属性投影到同一潜在空间”的直观理解。

<p align="center"><img src="../media/07_machine_learning/media/image10.jpeg" alt="" width="60%" /></p>
<p align="center">图例 7‑10 交替最小二乘法</p>


&emsp;&emsp;ALS 是一种迭代优化算法。训练时，它会先固定用户因子，更新项目因子；再固定项目因子，更新用户因子，如此交替进行，直到损失函数收敛或达到迭代上限。对于推荐问题，这种方法的优点在于能够较好地处理评分矩阵的稀疏性，并且天然适合并行化，因此长期都是分布式推荐系统里的经典方案。阅读下面的历史示例时，重点可以放在三件事上：评分数据如何组织、训练集与测试集如何拆分，以及模型如何从用户 - 物品交互中学习出潜在偏好结构。

&emsp;&emsp;下图概括了这个案例的基本流程：先加载评分数据并整理成 ALS 可接受的输入格式，再拆分训练集与测试集，接着训练矩阵分解模型，最后用测试数据评估预测效果。下面先从读取 `ratings.dat` 开始，其中每一行都包含用户 ID、电影 ID、评分和时间戳。

<p align="center"><img src="../media/07_machine_learning/media/image11.png" alt="" width="60%" /></p>
<p align="center">图例 7‑11 机器学习的工作流程</p>
...
&emsp;&emsp;第一步先把评分文件读成 `RDD[String]`：
...
&emsp;&emsp;接着把原始文本行解析成 `Rating` 对象，并把结果缓存下来。
...
&emsp;&emsp;`parseRating` 的作用很直接：把原始文本行解析成 `Rating(user, product, rating)`，让后续训练代码能够围绕“用户、物品、评分”这三个核心字段展开。这里先把结果缓存下来，是因为训练、评估和后面的推荐演示都会反复用到同一批评分数据。
...
&emsp;&emsp;接着把评分数据做一次简单的 Hold-Out 拆分：80% 用于训练，20% 用于测试。对推荐任务来说，这一步的目的不是追求某种固定比例，而是把“模型见过的数据”和“用来检验泛化效果的数据”分开，避免只在训练集上看起来很好。

&emsp;&emsp;使用 `ALS.train()` 可以训练一个新的用户因子矩阵和物品因子矩阵。下面这段代码传入的参数实际是 `rank = 10`、`iterations = 20`。在 ALS 里，最值得先关注的几个超参数是：

&emsp;&emsp;maxIter：要运行的最大迭代次数（默认为10）

&emsp;&emsp;rank：模型中潜在因子的数量（默认为10）

&emsp;&emsp;regParam：交替最小二乘中的正则化参数（默认为1.0）
...
&emsp;&emsp;模型训练好以后，评估的基本思路也很清楚：先从测试集里拿到用户和物品的组合，再让模型为这些组合补出预测评分，然后与真实评分对比。
...
&emsp;&emsp;接着调用 `model.predict()`，让模型为每个测试用户和影片组合生成预测评分。
...
&emsp;&emsp;为了把预测值和真实值一一对齐，这里把两边都整理成以 `(user, product)` 为键的键值对。这样后面做 `join` 时，就能直接比较同一用户对同一影片的“真实评分”和“预测评分”。
...
&emsp;&emsp;除了看整体误差，还可以单独挑出业务上更敏感的错误类型。下面把“真实评分很低，但模型却给出很高预测”的情况当成误报来检查，这类样本往往比普通误差更值得人工回看。
...
&emsp;&emsp;更常见的整体指标是平均绝对误差（MAE）。它把每条预测与真实评分之间的绝对偏差取平均，数值越小，说明模型给出的评分越接近真实评分。
...
&emsp;&emsp;最后这一段代码模拟了推荐系统里很常见的“新用户冷启动”流程：先收集少量显式评分，再把这些评分并入原始数据，重新训练模型，最后为这个新用户生成推荐列表。真实系统里未必每次都全量重训，但这个例子足够说明 ALS 如何把新增交互转成可用的推荐结果。
...
&emsp;&emsp;> 兼容性说明：下面的决策树示例使用 `spark.mllib` 的 `LabeledPoint` 与 RDD 风格 API，用于解释历史代码和算法参数。新项目建议优先使用 `spark.ml.classification.DecisionTreeClassifier` 配合 DataFrame/Pipeline。

&emsp;&emsp;如果按现代主线重做这个案例，通常会先把 CSV 读成 DataFrame，再用 `StringIndexer`、`VectorAssembler`、`DecisionTreeClassifier` 与 `BinaryClassificationEvaluator` 组织成一条 Pipeline。下面保留旧写法，主要是为了帮助你在存量项目中读懂特征准备、训练/测试拆分和模型评估这些基本步骤。

&emsp;&emsp;这个案例使用的是航班信息数据集。对于每一次航班，样本里都记录了日期、承运人、起降机场、计划起降时间、实际起降时间和飞行时长等字段：
...
&emsp;&emsp;表格 7‑3 数据描述

&emsp;&emsp;这个任务的目标是预测航班是否会明显延误。这里把“延误 40 分钟以上”记为 `Yes`，否则记为 `No`。训练时使用的特征包括日期、星期、计划起降时间、承运人、计划飞行时长以及起降机场等字段，本质上是在做一个典型的表格二分类问题。下面先从 CSV 加载并解析数据。
...
&emsp;&emsp;先定义一个 `Flight` case class，对应 CSV 里的一行航班记录：
...
&emsp;&emsp;再定义解析函数，把文本行转换成 `Flight`：

&emsp;&emsp;// function to parse input into Flight class
...
&emsp;&emsp;下面从 CSV 读取数据、解析并缓存；`first()` 只是为了快速确认解析结果是否符合预期。
...
&emsp;&emsp;要训练树模型，下一步需要把每条航班记录整理成“标签 + 特征向量”的形式。这里把延误超过 40 分钟记为正类，其余记为负类；日期、星期、计划起降时间、承运人、计划飞行时长以及起降机场等字段共同组成特征。由于这个历史示例使用的是 `spark.mllib` 的 RDD API，因此还需要先把承运人、始发机场、目的机场这类字符串字段手工编码成整数索引。

&emsp;&emsp;下面分别为承运人、始发机场和目的机场建立字符串到整数索引的映射：
...
&emsp;&emsp;有了这些映射之后，就可以把每条航班记录整理成“标签 + 数值特征”的中间表示：
...
&emsp;&emsp;从包含RDD的特征数组中，创建包含LabeledPoints数组的[RDD](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/mllib/regression/LabeledPoint.html)，其中定义了数据点的特征向量和标签：

&emsp;&emsp;//Making LabeledPoint of features - this is the training data for the
&emsp;&emsp;model
...
&emsp;&emsp;这里又做了一次样本重组：保留全部延误样本，并只抽取部分非延误样本，再把它们合并成新的训练候选集。这样做的动机，是在历史写法里用较简单的方式缓解类别不平衡，让模型不要被大量“未延误”样本完全主导。
...
&emsp;&emsp;然后再把这个重组后的数据拆成训练集和测试集：
...
&emsp;&emsp;接下来准备决策树训练所需的几个关键参数。这里最值得关注的是 `categoricalFeaturesInfo`：它告诉算法哪些特征应被当作类别特征处理，以及每个特征大概有多少个离散取值。对于树模型来说，这一步很重要，因为它直接影响节点如何做分裂。除此之外，`maxDepth` 控制树能长多深，`maxBins` 控制连续特征离散化时可用的桶数，`impurity` 则指定节点纯度的度量方式。把这些参数和训练数据一起交给 `DecisionTree.trainClassifier()` 后，就能得到一个历史风格的 `DecisionTreeModel`。

&emsp;&emsp;先把几个离散特征的取值范围告诉模型：这里分别为 `dofM`、`dofW`、`carrier`、`origin` 和 `dest` 建立类别信息映射。
...
&emsp;&emsp;`model.toDebugString` 会把整棵树的分裂路径打印出来，适合用来理解模型究竟依赖了哪些字段、树长到了多深，但在真实项目里通常还会结合特征重要性、混淆矩阵和业务指标一起看。接下来回到最基础的评估流程：在测试集上生成预测，再把预测标签和真实标签对比，计算错误率。这个历史示例里，错误率约为 31%。

&emsp;&emsp;下面在测试集上生成预测，并统计最基础的错误率：
