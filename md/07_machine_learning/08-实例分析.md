...
本节保留两个案例：一个围绕推荐系统，一个围绕分类问题。需要注意，案例中的部分代码仍使�?`spark.mllib` / RDD 风格 API，目的是帮助读者理解历史写法和存量系统；如果在 Spark 4.x 中新建项目，应优先选择 `spark.ml`、DataFrame �?Pipeline 的等价实现�?

### 7.8.1 预测用户偏好

> 兼容性说明：下面�?ALS 示例使用 RDD 风格写法，适合说明矩阵分解与旧项目代码结构。Spark 4.x 新项目建议优先参�?`org.apache.spark.ml.recommendation.ALS` �?DataFrame API�?

如果今天重新实现这一类推荐任务，通常会先把评分数据整理成包含 `user`、`item`、`rating` 三列�?DataFrame，再把特征清洗、训练与评估放进更一致的工程流程中。保留下面的写法，是为了帮助读者识别历史推荐系统代码最常见的组织方式�?
...
下面继续用电影推荐说明协同过滤。假�?Ted 喜欢电影 A、B、C，Carol 喜欢电影 B、C，那么系统就会尝试根据这些交互记录推�?Bob 可能喜欢什么。Spark 里的 ALS（Alternating Least Squares，交替最小二乘）正是围绕这类“用�?- 物品评分矩阵”构建的经典矩阵分解方法�?

ALS 的核心思路，是把原始的稀疏评分矩阵分解成两个更低维的稠密矩阵：一个表示用户在若干潜在因子上的偏好强度，另一个表示项目在同一组潜在因子上的表现。用户和项目在这些潜在因子上的匹配程度，决定了模型给出的预测评分。图�?7�?0 展示了这种“把用户兴趣和项目属性投影到同一潜在空间”的直观理解�?
...
下图概括了这个案例的基本流程：先加载评分数据并整理成 ALS 可接受的输入格式，再拆分训练集与测试集，接着训练矩阵分解模型，最后用测试数据评估预测效果。下面先从读�?`ratings.dat` 开始，其中每一行都包含用户 ID、电�?ID、评分和时间戳�?
...
接着把评分数据做一次简单的 Hold-Out 拆分�?0% 用于训练�?0% 用于测试。对推荐任务来说，这一步的目的不是追求某种固定比例，而是把“模型见过的数据”和“用来检验泛化效果的数据”分开，避免只在训练集上看起来很好�?

使用 `ALS.train()` 可以训练一个新的用户因子矩阵和物品因子矩阵。下面这段代码传入的参数实际�?`rank = 10`、`iterations = 20`。在 ALS 里，最值得先关注的几个超参数是�?

maxIter：要运行的最大迭代次数（默认�?0�?

rank：模型中潜在因子的数量（默认�?0�?
...
为了把预测值和真实值一一对齐，这里把两边都整理成�?`(user, product)` 为键的键值对。这样后面做 `join` 时，就能直接比较同一用户对同一影片的“真实评分”和“预测评分”�?
...
最后这一段代码模拟了推荐系统里很常见的“新用户冷启动”流程：先收集少量显式评分，再把这些评分并入原始数据，重新训练模型，最后为这个新用户生成推荐列表。真实系统里未必每次都全量重训，但这个例子足够说�?ALS 如何把新增交互转成可用的推荐结果�?
...
> 兼容性说明：下面的决策树示例使用 `spark.mllib` �?`LabeledPoint` �?RDD 风格 API，用于解释历史代码和算法参数。新项目建议优先使用 `spark.ml.classification.DecisionTreeClassifier` 配合 DataFrame/Pipeline�?

如果按现代主线重做这个案例，通常会先�?CSV 读成 DataFrame，再�?`StringIndexer`、`VectorAssembler`、`DecisionTreeClassifier` �?`BinaryClassificationEvaluator` 组织成一�?Pipeline。下面保留旧写法，主要是为了帮助你在存量项目中读懂特征准备、训�?测试拆分和模型评估这些基本步骤�?
...
| dOfW (String)           | 星期�?        | 4      |
| carrier (String)        | 运营商代�?      | AA     |
| tailNum (String)        | 飞机的唯一标识�?尾号 | N787AA |
| flnum(Int)              | 航班�?        | 21     |
| org\_id(String)         | 始发机场编号      | 12478  |
| origin(String)          | 始发机场代码      | JFK    |
| dest\_id (String)       | 目的地机场编�?    | 12892  |
| dest (String)           | 目的地机场代�?    | LAX    |
...
这个任务的目标是预测航班是否会明显延误。这里把“延�?40 分钟以上”记�?`Yes`，否则记�?`No`。训练时使用的特征包括日期、星期、计划起降时间、承运人、计划飞行时长以及起降机场等字段，本质上是在做一个典型的表格二分类问题。下面先�?CSV 加载并解析数据�?
...
先定义一�?`Flight` case class，对�?CSV 里的一行航班记录：
...
下面�?CSV 读取数据、解析并缓存；`first()` 只是为了快速确认解析结果是否符合预期�?
...
要训练树模型，下一步需要把每条航班记录整理成“标�?+ 特征向量”的形式。这里把延误超过 40 分钟记为正类，其余记为负类；日期、星期、计划起降时间、承运人、计划飞行时长以及起降机场等字段共同组成特征。由于这个历史示例使用的�?`spark.mllib` �?RDD API，因此还需要先把承运人、始发机场、目的机场这类字符串字段手工编码成整数索引�?
...
有了这些映射之后，就可以把每条航班记录整理成“标�?+ 数值特征”的中间表示�?
...
接下来准备决策树训练所需的几个关键参数。这里最值得关注的是 `categoricalFeaturesInfo`：它告诉算法哪些特征应被当作类别特征处理，以及每个特征大概有多少个离散取值。对于树模型来说，这一步很重要，因为它直接影响节点如何做分裂。除此之外，`maxDepth` 控制树能长多深，`maxBins` 控制连续特征离散化时可用的桶数，`impurity` 则指定节点纯度的度量方式。把这些参数和训练数据一起交�?`DecisionTree.trainClassifier()` 后，就能得到一个历史风格的 `DecisionTreeModel`�?

先把几个离散特征的取值范围告诉模型：这里分别�?`dofM`、`dofW`、`carrier`、`origin` �?`dest` 建立类别信息映射�?
