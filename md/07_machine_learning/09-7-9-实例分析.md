...
图例 7‑11 概括了这个案例的基本流程：先加载评分数据并整理成 ALS 可接受的输入格式，再拆分训练集与测试集，接着训练矩阵分解模型，最后用测试数据评估预测效果。下面的示例读取 `ratings.dat`，其中每一行都包含用户 ID、电影 ID、评分和时间戳。
...
接下来把评分数据做一次简单的 Hold-Out 拆分：80% 用于训练，20% 用于测试。对推荐任务来说，这一步的目的不是追求某种固定比例，而是把“模型见过的数据”和“用来检验泛化效果的数据”分开，避免只在训练集上看起来很好。
...
最后这段代码模拟了推荐系统里很常见的“新用户冷启动”流程：先收集少量显式评分，再把这些评分并入原始数据，重新训练模型，最后为这个新用户生成推荐列表。真实系统里未必每次都全量重训，但这个例子足够说明 ALS 如何把新增交互转成可用的推荐结果。
...
`model.toDebugString` 会把整棵树的分裂路径打印出来，适合用来理解模型究竟依赖了哪些字段、树长到了多深，但在真实项目里通常还会结合特征重要性、混淆矩阵和业务指标一起看。下面这段代码则回到最基础的评估流程：在测试集上生成预测，再把预测标签和真实标签对比，计算错误率。这个历史示例里，错误率约为 31%。
