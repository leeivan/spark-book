...
�?Spark 中，机器学习管道主要由三类对象协作完成：Transformer 负责把一�?DataFrame 转成另一�?DataFrame；Estimator 负责在训练数据上调用 `fit()` 生成模型；Evaluator 负责度量模型效果并为调参提供依据。把这些部件按顺序组合起来，才能让训练集、验证集和线上推理使用同一套前处理逻辑�?

�?Spark 4.x 的实践角度看，Pipeline 不是“可选高级话题”，而是 `spark.ml` 的默认工作方式。只要任务包含特征工程和模型训练，就应该优先思考如何把流程写成可复用的 Pipeline�?
...
Spark 的机器学习管�?API 位于 `org.apache.spark.ml` 包中，建立在 DataFrame 之上，能够与特征工程、模型、评估器和参数网格搜索自然组合。这也是为什么本章把 Pipeline 视为新项目主线，而把 RDD 风格 API 视为兼容材料�?
...
下面再按组件角色展开看，就更容易理解为什�?Transformer、Estimator �?Evaluator 能被自然地拼成同一条链路�?
...
在真实项目里，模型训练通常不是“一个算法调用”就结束了，而是由一串有顺序约束的步骤组成。例如在文本分类任务中，你可能先做分词，再做向量化，最后训练分类器；在推荐或风控任务中，则常常先做数据清洗、类别编码、特征拼接，再进入模型训练和评估。Spark 把这类流程统一抽象�?Pipeline：它由多�?`PipelineStage` 顺序组成，每个阶段要么负责转换数据，要么负责根据训练数据拟合模型�?

运行时可以把 Pipeline 理解成一条固定的数据处理链。对�?Transformer，Spark 会直接调�?`transform()` 把一�?DataFrame 变成另一�?DataFrame；对�?Estimator，Spark 会先调用 `fit()` 产出训练后的模型，再把这个模型作为后续链路中�?Transformer 使用。训练完成后，整条链路会固化成一�?`PipelineModel`，从而让训练集、验证集和线上推理都复用同一套预处理步骤。下图（图例 7�?）展示了这条链路如何作用在训练数据上�?

![L Pipeline Example](../media/07_machine_learning/media/image8.jpeg)

图例 7�? 训练模型阶段使用的机器学习管�?

图例 7�? 中，`Tokenizer` �?`HashingTF` 负责把原始文本逐步变成可训练的特征列，`LogisticRegression` 则负责在这些特征上拟合分类模型。调�?`Pipeline.fit()` 时，前面的转换器依次改写 DataFrame，最后由估算器产出训练后的模型；等整条链路完成拟合后，Spark 会返回一�?`PipelineModel`。随后，测试集或线上数据只需要调用这�?`PipelineModel` �?`transform()`，就能沿着相同的步骤得到预测结果。这样做的价值在于：训练时用什么预处理逻辑，推理时就沿用什么逻辑，从而减少“训练和上线不一致”的风险�?

![L PipelineModel Example](../media/07_machine_learning/media/image9.jpeg)

图例 7�? 机器学习管道应用在测试数据的过程

Pipeline 中的阶段按顺序放在一个数组里。最常见的是线性链路，也就是后一阶段直接消费前一阶段产出的列；如果数据流能组成有向无环图，也可以构造更复杂的非线�?Pipeline，但这时就需要格外注意各阶段的输入列、输出列和先后顺序�?

由于 Pipeline 要处理的是运行时 DataFrame，而不是编译期静态类型，很多合法性检查都会推迟到运行阶段完成。Spark 会根�?DataFrame �?schema 校验列名、列类型和阶段依赖是否匹配。因此，给每个阶段明确设置输入列和输出列，是保持管道可读性与可维护性的关键做法�?

另外，Pipeline 里的阶段实例应该保持唯一。如果你需要两次使�?`HashingTF`，最好创建两个独立实例并分别配置各自参数，而不是把同一个对象重复塞进阶段数组。这样做既能避免 ID 冲突，也更方便排查参数配置问题�?

估算器和转换器使用统一的API来指定参数。Param是具有独立文件的命名参数，ParamMap是一组键值对(参数,�?，将参数传递给算法有两种主要方法：

�?）为实例设置参数，如果lr是LogisticRegression的实例，则可以调用lr.setMaxIter(10)使lr.fit()最多使�?0次迭代�?

�?）将ParamMap传递给fit()或transform()方法。ParamMap中的任何参数都将覆盖以前通过setter方法指定的参数�?

参数总是绑定在某个估算器或转换器的具体实例上。如果有两个 `LogisticRegression` 实例 `lr1` �?`lr2`，就可以分别为它们构造不同的 `maxIter` 参数，例�?`ParamMap(lr1.maxIter -> 10, lr2.maxIter -> 20)`。当一�?Pipeline 里同时出现多个同类阶段时，这种“参数绑定到实例”的机制尤其重要。除此之外，Pipeline 和模型本身也可以保存到存储中，供后续加载和复用。下面这�?Scala REPL 示例展示一条典型文本处�?Pipeline 如何被定义、拟合、保存、加载并重新用于预测�?
...
本节介绍如何�?`spark.ml` 中做模型选择和超参数调优。现实中的调优很少只针对某一个算法参数，而更常见的是把整�?Pipeline 当成一个整体来优化：特征工程、模型参数、评估指标和数据切分方式一起决定最终效果。Spark 提供的核心工具主要有 `CrossValidator` �?`TrainValidationSplit`；其中，估算器代表待调优的算法或管道，`ParamMap`/`ParamGridBuilder` 负责列出候选参数组合，评估器（`Evaluator`）则负责衡量模型在验证数据上的表现。把这些组件组合起来后，整体流程可以概括为：

�?）将输入的数据按照一定的比例和方法分成独立的训练和测试数据集�?

�?）对于每个训练数据和测试数据的组合，遍历ParamMap中的参数，使用这些参数训练估算器得到模型，并使用评估器验证模型的性能�?
...
从工程取舍看，`CrossValidator` �?`TrainValidationSplit` 的差别主要不在“能不能调参”，而在“愿意为稳定性付出多少训练成本”。如果数据量较大、参数网格已经不少，或者当前阶段更关注快速收敛到一个可用版本，那么 `TrainValidationSplit` 往往更经济；如果数据规模还可控，但你担心一次随机切分带来的波动，或者需要更稳地比较多组候选参数，那么 `CrossValidator` 更合适。两者都可以直接包住整条 Pipeline，而不是只针对最后一�?Estimator 单独调参�?

带着这个判断再往下看，`CrossValidator` 更像“用更多训练成本换更稳的参数比较”，�?`TrainValidationSplit` 则更像“先用一次划分快速筛出一批可行配置”。两者的差别，本质上是在训练预算和结果稳定性之间做选择�?

#### 7.7.3.1 CrossValidator

`CrossValidator` 会先把数据集拆成若干折（fold），轮流拿其中一部分做验证集，其余部分做训练集。比如当 `k = 3` 时，它会形成 3 �?`(training, test)` 数据对，每一组都用大�?`2/3` 的数据训练、`1/3` 的数据验证。对于某个给定的 `ParamMap`，Spark 会在所有折上重复拟合并计算评估指标，最后取平均值；确定最佳参数组合后，再用整个数据集重新拟合一次最终模型�?

下面这组 Scala REPL 示例演示如何�?`CrossValidator` 在参数网格中选择更优组合。需要先有一个现实预期：交叉验证的成本通常不低。在这个例子里，`hashingTF.numFeatures` �?3 个候选值，`lr.regParam` �?2 个候选值，再乘�?2 折交叉验证，总共就要训练 `(3 * 2) * 2 = 12` 个模型。真实项目里，参数更多、折数更高时，成本还会继续上升；但相应地，它通常也比手动拍脑袋调参更稳健�?
...
hashingTF.numFeatures�?个值，lr.regParam�?个值，这个网格�? x 2 =
...
�?`CrossValidator` 外，Spark 还提�?`TrainValidationSplit` 用于超参数选择。它只对每个参数组合做一次训�?验证拆分，因此计算成本明显低�?`k` 折交叉验证；代价是评估结果更依赖这一次拆分本身，数据量较小时稳定性往往不如 `CrossValidator`。它的工作方式很直接：先�?`trainRatio` 把数据切成一�?`(training, test)`，例如当 `trainRatio = 0.75` 时，75% 的数据用于训练，25% 用于验证；在所有参数组合上评估完之后，再用最佳参数和整份数据重新拟合最终模型�?
...
TrainValidationSplit会调用所有的参数组合，使用评估器确定最好的模型。其包括线性回归评估器，评估器参数集合和估算器�?0%的数据被用来作为训练数据�?0%作为测试数据�?
