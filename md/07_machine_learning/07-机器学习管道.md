...
从工程取舍看，`CrossValidator` 和 `TrainValidationSplit` 的差别主要不在“能不能调参”，而在“愿意为稳定性付出多少训练成本”。如果数据量较大、参数网格已经不少，或者当前阶段更关注快速收敛到一个可用版本，那么 `TrainValidationSplit` 往往更经济；如果数据规模还可控，但你担心一次随机切分带来的波动，或者需要更稳地比较多组候选参数，那么 `CrossValidator` 更合适。两者都可以直接包住整条 Pipeline，而不是只针对最后一个 Estimator 单独调参。

带着这个判断再往下看，`CrossValidator` 更像“用更多训练成本换更稳的参数比较”，而 `TrainValidationSplit` 则更像“先用一次划分快速筛出一批可行配置”。两者的差别，本质上是在训练预算和结果稳定性之间做选择。
